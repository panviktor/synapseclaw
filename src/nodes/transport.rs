...
/// - `X-SynapseClaw-Timestamp` — unix epoch seconds
/// - `X-SynapseClaw-Nonce` — random UUID v4
/// - `X-SynapseClaw-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-SynapseClaw-Timestamp", timestamp.to_string())
            .header("X-SynapseClaw-Nonce", &nonce)
            .header("X-SynapseClaw-Signature", &signature)
