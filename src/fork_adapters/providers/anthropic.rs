...
impl AnthropicProvider {
        ...
        for msg in messages {
            match msg.role.as_str() {
                ...
                _ => {
                    ...
                    let (text, image_refs) =
                        crate::fork_adapters::multimodal::parse_image_markers(&msg.content);
    ...
    fn http_client(&self) -> Client {
        fork_config::schema::build_runtime_proxy_client_with_timeouts("provider.anthropic", 120, 10)
    }
}
...
mod tests {
    use super::*;
    use crate::fork_adapters::auth::anthropic_token::{detect_auth_kind, AnthropicAuthKind};
    ...
    async fn chat_with_system_fails_without_key() {
        let p = AnthropicProvider::new(None);
        let result = p
            .chat_with_system(Some("You are SynapseClaw"), "hello", "claude-3-opus", 0.7)
            .await;
        assert!(result.is_err());
    }
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are SynapseClaw".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are SynapseClaw\""));
    }
