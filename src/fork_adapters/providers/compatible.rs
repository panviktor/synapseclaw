...
impl OpenAiCompatibleProvider {
    ...
    fn http_client(&self) -> Client {
        ...
        if has_user_agent || has_extra_headers {
            ...
            let builder =
                fork_config::schema::apply_runtime_proxy_to_builder(builder, "provider.compatible");
    ...
    fn tool_specs_to_openai_format(
        tools: &[crate::fork_adapters::tools::ToolSpec],
...
impl OpenAiCompatibleProvider {
    ...
    fn convert_tool_specs(
        tools: Option<&[crate::fork_adapters::tools::ToolSpec]>,
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions =
            crate::fork_adapters::providers::traits::build_tool_instructions_text(tools);
...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(&self) -> crate::fork_adapters::providers::traits::ProviderCapabilities {
        crate::fork_adapters::providers::traits::ProviderCapabilities {
...
mod tests {
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are SynapseClaw".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn prompt_guided_tool_fallback_injects_system_instruction() {
        ...
        let tools = vec![crate::fork_adapters::tools::ToolSpec {
    ...
    fn user_agent_constructor_keeps_native_tool_calling_enabled() {
        let p = OpenAiCompatibleProvider::new_with_user_agent(
            ...
            Some("k"),
            AuthStyle::Bearer,
            "synapseclaw-test/1.0",
        );
        let caps = <OpenAiCompatibleProvider as Provider>::capabilities(&p);
        assert!(caps.native_tool_calling);
        assert!(!caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("synapseclaw-test/1.0"));
    }
    ...
    fn user_agent_and_vision_constructor_preserves_capability_flags() {
        let p = OpenAiCompatibleProvider::new_with_user_agent_and_vision(
            ...
            Some("k"),
            AuthStyle::Bearer,
            "synapseclaw-test/vision",
            true,
        );
        let caps = <OpenAiCompatibleProvider as Provider>::capabilities(&p);
        assert!(caps.native_tool_calling);
        assert!(caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("synapseclaw-test/vision"));
    }
    ...
    fn tool_specs_convert_to_openai_format() {
        let specs = vec![crate::fork_adapters::tools::ToolSpec {
    ...
    fn with_extra_headers_sets_headers() {
        ...
        headers.insert("X-Title".to_string(), "synapseclaw".to_string());
        headers.insert(
        ...
        assert_eq!(p.extra_headers.get("X-Title").unwrap(), "synapseclaw");
        assert_eq!(
