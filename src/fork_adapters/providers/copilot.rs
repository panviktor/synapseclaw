...
/// Tokens are cached to `~/.config/synapseclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "synapseclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("synapseclaw-copilot-{user}"))
            });
    ...
    fn http_client(&self) -> Client {
        fork_config::schema::build_runtime_proxy_client_with_timeouts("provider.copilot", 120, 10)
    }
