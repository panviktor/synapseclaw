            ...
            .await
            {
                Ok(resumed) => {
                    ...
                    for turn in &resumed.transcript {
                        agent.push_history(
                            crate::fork_adapters::providers::ConversationMessage::Chat(
...
fn replay_messages_into_agent(
    ...
    use crate::fork_adapters::providers::{ChatMessage, ConversationMessage};
...
async fn handle_chat_send_rpc(
    ...
    match result {
        ...
        Err(e) => {
            ...
            let sanitized = crate::fork_adapters::providers::sanitize_api_error(&msg);
...
fn persist_usage_memory(
    state: &AppState,
    session_key: &str,
    usage: Option<&crate::fork_adapters::providers::traits::TokenUsage>,
) {
    if let Some(u) = usage {
...
async fn persist_tool_events(
    state: &AppState,
    session_key: &str,
    history: &[crate::fork_adapters::providers::ConversationMessage],
) {
    use crate::fork_adapters::providers::ConversationMessage;
...
pub(crate) async fn summarize_session_if_needed(state: &AppState, session_key: &str) {
    ...
    let (summary_model, temperature, provider) = {
        ...
        let prov: std::sync::Arc<dyn crate::fork_adapters::providers::Provider> =
            if let Some(ref provider_name) = sc.provider {
                let opts = crate::fork_adapters::providers::provider_runtime_options_from_config(
                    &config_guard,
                );
                let api_key = sc
                    .api_key_env
                ...
                match crate::fork_adapters::providers::create_provider_with_options(
                    provider_name,
                    api_key.as_deref(),
                    &opts,
                ) {
