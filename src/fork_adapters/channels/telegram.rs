...
use fork_security::pairing::PairingGuard;
...
mod tests {
    ...
    async fn e2e_live_voice_transcription_and_reply_cache() {
        ...
        let transcript: String = crate::fork_adapters::channels::transcription::transcribe_audio(
            audio_data,
            "hello.mp3",
            &config,
        )
    ...
    fn e2e_attachment_saves_file_and_formats_content() {
        ...
        let doc_msgs = vec![crate::fork_adapters::providers::ChatMessage::user(
            doc_content,
        )];
        assert_eq!(
        ...
        let photo_msgs = vec![crate::fork_adapters::providers::ChatMessage::user(
            photo_content.clone(),
        )];
        assert_eq!(
        ...
        let cap_msgs = vec![crate::fork_adapters::providers::ChatMessage::user(
            captioned.clone(),
        )];
        assert_eq!(
        ...
        let md_msgs = vec![crate::fork_adapters::providers::ChatMessage::user(
            md_content,
        )];
        assert_eq!(
    ...
    fn groq_provider_rejects_photo_with_vision_error() {
        use crate::fork_adapters::providers::compatible::{AuthStyle, OpenAiCompatibleProvider};
