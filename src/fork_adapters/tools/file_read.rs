...
mod tests {
    ...
    async fn e2e_agent_file_read_pdf_extraction() {
        ...
        use crate::fork_adapters::providers::{ChatResponse, Provider, ToolCall};
        use e2e_helpers::*;

        // ── Set up workspace with PDF fixture ──
        let workspace = std::env::temp_dir().join("synapseclaw_test_e2e_file_read_pdf");
    ...
    async fn e2e_agent_file_read_lossy_binary() {
        ...
        let workspace = std::env::temp_dir().join("synapseclaw_test_e2e_file_read_lossy");
        let _ = tokio::fs::remove_dir_all(&workspace).await;
        tokio::fs::create_dir_all(&workspace).await.unwrap();
    ...
    ///
    /// Requires valid OAuth credentials in `~/.synapseclaw/`.
    ...
    async fn e2e_live_file_read_pdf() {
        ...
        use crate::fork_adapters::providers::openai_codex::OpenAiCodexProvider;
        use crate::fork_adapters::providers::{Provider, ProviderRuntimeOptions};
        use e2e_helpers::*;

        // ── Set up workspace with PDF fixture ──
        let workspace = std::env::temp_dir().join("synapseclaw_test_e2e_live_file_read_pdf");
        let _ = tokio::fs::remove_dir_all(&workspace).await;
        tokio::fs::create_dir_all(&workspace).await.unwrap();
    ...
    async fn file_read_blocks_null_byte_in_path() {
        let dir = std::env::temp_dir().join("synapseclaw_test_file_read_null_byte");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_allowed_root_with_workspace_only() {
        let root = std::env::temp_dir().join("synapseclaw_test_file_read_allowed_root");
        let workspace = root.join("workspace");
