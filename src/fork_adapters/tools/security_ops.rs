...
use fork_security::playbook::{
    evaluate_step, load_playbooks, severity_level, Playbook, StepStatus,
};
use fork_security::vulnerability::{generate_summary, parse_vulnerability_json};
    ...
    fn generate_report(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let report = format!(
             ...
             **Generated:** {}\n\n\
             ...
             ---\n\
             *Report generated by SynapseClaw MCSS Agent*\n",
...
mod tests {
    ...
    async fn generate_report_produces_markdown() {
        ...
        assert!(result.success);
        assert!(result.output.contains("SynapseClaw Corp"));
