...
use crate::fork_adapters::gateway::nodes::{NodeInvocation, NodeRegistry};
...
mod tests {
    use super::*;
    use crate::fork_adapters::gateway::nodes::{NodeCapability, NodeInfo, NodeRegistry};
        ...
        tokio::spawn(async move {
            if let Some(invocation) = invoke_rx.recv().await {
                let _ = invocation.response_tx.send(
                    crate::fork_adapters::gateway::nodes::NodeInvocationResult {
                        success: true,
