...
use crate::fork_core::ports::agent_runtime::AgentRuntimePort;
...
pub struct InboundMessageConfig {
    ...
    pub query_classification: crate::config::QueryClassificationConfig,
...
async fn handle_regular_message(
    ...
    if config.query_classification.enabled {
        if let Some(hint) =
            crate::agent::classifier::classify(&config.query_classification, content)
        {
...
async fn execute_agent_turn(
    ...
    config: &InboundMessageConfig,
    ports: &InboundMessagePorts,
    route: &crate::fork_core::ports::route_selection::RouteSelection,
    ...
    match result {
        ...
        Err(e) => {
            ...
            ports
                .history
                .rollback_last_turn(conversation_key, content);
            let safe_err = crate::providers::sanitize_api_error(&err_str);
...
mod tests {
    ...
    impl AgentRuntimePort for MockRuntime {
        ...
        ) -> Result<crate::fork_core::ports::agent_runtime::AgentTurnResult> {
            Ok(crate::fork_core::ports::agent_runtime::AgentTurnResult {
    ...
    impl ChannelRegistryPort for MockRegistry {
        fn resolve(&self, _n: &str) -> Result<Arc<dyn crate::channels::traits::Channel>> { anyhow::bail!("mock") }
        fn capabilities(&self, _n: &str) -> Vec<ChannelCapability> {
            vec![ChannelCapability::SendText, ChannelCapability::RuntimeCommands]
        }
        async fn deliver(&self, _i: &crate::fork_core::domain::channel::OutboundIntent) -> Result<()> { Ok(()) }
    }
