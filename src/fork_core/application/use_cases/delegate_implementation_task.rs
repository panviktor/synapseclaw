...
use crate::fork_core::ports::coding_worker::CodingWorkerPort;
...
mod tests {
    ...
    impl CodingWorkerPort for MockWorker {
        ...
        async fn get_events(
            ...
            _run_id: &str,
            _limit: usize,
        ) -> Result<Vec<crate::fork_core::domain::implementation::ImplementationEvent>> {
            Ok(vec![])
        }
    ...
    impl RunStorePort for MockRunStore {
        ...
        async fn append_event(&self, _event: &crate::fork_core::domain::run::RunEvent) -> Result<()> { Ok(()) }
        async fn get_events(&self, _run_id: &str, _limit: usize) -> Vec<crate::fork_core::domain::run::RunEvent> { vec![] }
    }
