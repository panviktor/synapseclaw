...
Examples:
  ...
  synapseclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  synapseclaw hardware info --chip STM32F401RETx")]
    Info {
...
Examples:
  ...
  synapseclaw peripheral add rpi-gpio native
  synapseclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash SynapseClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash SynapseClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  synapseclaw peripheral flash
  synapseclaw peripheral flash --port /dev/cu.usbmodem12345
  synapseclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash SynapseClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
