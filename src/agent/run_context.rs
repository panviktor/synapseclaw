...
//!
//! Created by the caller (e.g. gateway inbox processor), threaded through the
//! tool call loop, and inspected after the run completes.  This is the
//! stepping-stone toward Phase 4.0's first-class `Run` object.
...
impl RunContext {
    ...
    ///   `session_id`.
    ///
    /// This covers both explicit reply paths the model can take (#4, #5 from
    /// the security audit).
...
mod tests {
    ...
    fn per_session_reply_via_agents_send_result() {
        let ctx = RunContext::new();
        // agents_send with kind=result is semantically equivalent to agents_reply
        ctx.record_tool_call(
            "agents_send",
            true,
