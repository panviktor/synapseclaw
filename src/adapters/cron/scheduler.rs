...
pub async fn run(
    config: Config,
    delivery_service: Arc<synapse_core::application::services::delivery_service::DeliveryService>,
    ...
    crate::adapters::health::mark_component_ok(SCHEDULER_COMPONENT);
    ...
    loop {
        ...
        let jobs = match due_jobs(&config, Utc::now()) {
            Ok(jobs) => jobs,
            Err(e) => {
                crate::adapters::health::mark_component_error(SCHEDULER_COMPONENT, e.to_string());
...
pub async fn execute_job_now(
    config: &Config,
    job: &CronJob,
    agent_runner: &dyn synapse_core::ports::agent_runner::AgentRunnerPort,
) -> (bool, String) {
...
async fn persist_job_result(
    ...
    let core_delivery = crate::adapters::daemon::cron_delivery_config_from(&job.delivery);
...
async fn run_job_command_with_timeout(
    ...
    if let Err(error) = crate::adapters::cron::validate_shell_command_with_security(
        security,
        &job.command,
        approved,
    ) {
...
mod tests {
    ...
    fn noop_runner() -> &'static dyn synapse_core::ports::agent_runner::AgentRunnerPort {
        &NoopRunner
    }
    ...
    struct FailRunner;
    #[async_trait::async_trait]
    impl synapse_core::ports::agent_runner::AgentRunnerPort for FailRunner {
        async fn run(
            &self,
            _: Option<String>,
            _: Option<String>,
    ...
    fn fail_runner() -> &'static dyn synapse_core::ports::agent_runner::AgentRunnerPort {
        &FailRunner
    }
    ...
    ) -> Arc<synapse_core::application::services::delivery_service::DeliveryService> {
        let registry: Arc<dyn synapse_core::ports::channel_registry::ChannelRegistryPort> =
            Arc::new(
                crate::adapters::channels::registry::CachedChannelRegistry::new(Config::default()),
            );
        Arc::new(
            synapse_core::application::services::delivery_service::DeliveryService::new(registry),
        )
    }
    ...
    fn test_job(command: &str) -> CronJob {
        CronJob {
            id: "test-job".into(),
            expression: "* * * * *".into(),
            schedule: crate::adapters::cron::Schedule::Cron {
                expr: "* * * * *".into(),
                tz: None,
            },
    ...
    async fn run_job_command_blocks_readonly_mode() {
        let tmp = TempDir::new().unwrap();
        let mut config = test_config(&tmp).await;
        config.autonomy.level = synapse_core::domain::config::AutonomyLevel::ReadOnly;
    ...
    async fn process_due_jobs_marks_component_ok_even_when_idle() {
        ...
        crate::adapters::health::mark_component_error(&component, "pre-existing error");
        process_due_jobs(
            &config,
            &security,
            Vec::new(),
    ...
    async fn persist_job_result_success_deletes_one_shot() {
        ...
        let job = cron::add_agent_job(
            &config,
            Some("one-shot".into()),
            crate::adapters::cron::Schedule::At { at },
            "Hello",
    ...
    async fn persist_job_result_delivery_failure_non_best_effort_marks_error() {
        ...
        let job = cron::add_agent_job(
            ...
            crate::adapters::cron::Schedule::Cron {
                expr: "*/5 * * * *".into(),
                tz: None,
            },
            "deliver this",
    ...
    fn subprocess_agent_job(prompt: &str) -> CronJob {
        CronJob {
            ...
            schedule: crate::adapters::cron::Schedule::At {
