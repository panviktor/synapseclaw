...
use crate::adapters::auth::AuthService;
...
pub struct ProviderRuntimeOptions {
    ...
    pub synapseclaw_dir: Option<PathBuf>,
    ...
    /// These are merged from the config file and `SYNAPSECLAW_EXTRA_HEADERS` env var.
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["SYNAPSECLAW_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        "ollama" => {
                ...
                let env_url = std::env::var("SYNAPSECLAW_PROVIDER_URL").ok();
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `synapseclaw onboard` to reconfigure.\n\
...
/// Return the list of all known providers for display in `synapseclaw providers list`.
///
...
mod tests {
    ...
    fn resolve_qwen_oauth_context_prefers_explicit_override() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/synapseclaw-qwen-oauth-home-{}", std::process::id());
    ...
    fn resolve_qwen_oauth_context_uses_env_token_and_resource_url() {
        ...
        let fake_home = format!(
            "/tmp/synapseclaw-qwen-oauth-home-{}-env",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, Some("oauth-token"));
    ...
    fn resolve_qwen_oauth_context_reads_cached_credentials_file() {
        let _env_lock = env_lock();
        let fake_home = format!(
            "/tmp/synapseclaw-qwen-oauth-home-{}-file",
            std::process::id()
        );
    ...
    fn resolve_qwen_oauth_context_placeholder_does_not_use_dashscope_fallback() {
        let _env_lock = env_lock();
        let fake_home = format!(
            "/tmp/synapseclaw-qwen-oauth-home-{}-placeholder",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _synapseclaw_guard = EnvGuard::set("SYNAPSECLAW_API_KEY", None);
    ...
    fn resilient_provider_ignores_duplicate_and_invalid_fallbacks() {
        let reliability = synapse_core::config::schema::ReliabilityConfig {
            provider_retries: 1,
    ...
    fn resilient_provider_errors_for_invalid_primary() {
        let reliability = synapse_core::config::schema::ReliabilityConfig::default();
        let provider = create_resilient_provider(
            "totally-invalid",
            Some("provider-test-credential"),
            None,
    ...
    fn provider_runtime_options_extra_headers_passed_through() {
        ...
        extra_headers.insert("X-Title".to_string(), "synapseclaw".to_string());
    ...
    fn env_provider_url_overrides_api_url() {
        std::env::set_var("SYNAPSECLAW_PROVIDER_URL", "http://env-ollama:11434");
        ...
        std::env::remove_var("SYNAPSECLAW_PROVIDER_URL");
    }
}
