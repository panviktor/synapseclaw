...
use synapse_core::domain::conversation::{
...
async fn ensure_session(state: &AppState, session_key: &str) -> anyhow::Result<()> {
    ...
    let (label, msg_count, input_tok, output_tok, current_goal, session_summary) =
        if let Some(store) = state.conversation_store.as_ref() {
            ...
            match synapse_core::application::use_cases::resume_conversation::execute(
                store.as_ref(),
                session_key,
            )
            .await
            {
                Ok(resumed) => {
                    ...
                    for turn in &resumed.transcript {
                        agent.push_history(crate::adapters::providers::ConversationMessage::Chat(
                            crate::adapters::providers::ChatMessage {
    ...
    if need_persist {
        if let Some(store) = state.conversation_store.as_ref() {
            let session =
                synapse_core::application::services::conversation_service::new_web_session(
                    session_key,
                    None,
                );
...
fn replay_messages_into_agent(
    ...
    use crate::adapters::providers::{ChatMessage, ConversationMessage};
...
async fn handle_chat_send_rpc(
    ...
    let run_id = if let Some(store) = state.run_store.as_ref() {
        match synapse_core::application::use_cases::start_conversation_run::create_and_track_run(
            state
                .conversation_store
                .as_deref()
    ...
    match result {
        Ok(response) => {
            ...
            // Phase 4.0 Slice 3: finalize via conversation_service
            {
                ...
                {
                    let _ = synapse_core::application::use_cases::start_conversation_run::finalize_success(
        ...
        Err(e) => {
            ...
            if msg == "aborted" {
                ...
                if let (Some(cs), Some(rs)) =
                    (state.conversation_store.as_ref(), state.run_store.as_ref())
                {
                    let _ = synapse_core::application::use_cases::start_conversation_run::finalize_interrupted(
            ...
            let sanitized = crate::adapters::providers::sanitize_api_error(&msg);
            ...
            {
                let _ =
                    synapse_core::application::use_cases::start_conversation_run::finalize_failure(
                        rs.as_ref(),
                        cs.as_ref(),
                        &session_key,
                        &run_id,
...
async fn handle_sessions_new(
    ...
    let session_key =
        synapse_core::application::services::conversation_service::new_web_session_key(
            token_prefix,
        );
...
async fn handle_sessions_delete(
    ...
    if let Some(store) = state.conversation_store.as_ref() {
        let _ = synapse_core::application::services::conversation_service::delete_session(
            store.as_ref(),
            key,
        )
        .await;
...
async fn handle_sessions_reset(
    ...
    if let Some(store) = state.conversation_store.as_ref() {
        let _ = synapse_core::application::services::conversation_service::reset_session(
            store.as_ref(),
            key,
        )
        .await;
...
fn persist_usage_memory(
    state: &AppState,
    session_key: &str,
    usage: Option<&crate::adapters::providers::traits::TokenUsage>,
) {
    if let Some(u) = usage {
...
async fn persist_tool_events(
    state: &AppState,
    session_key: &str,
    history: &[crate::adapters::providers::ConversationMessage],
) {
    use crate::adapters::providers::ConversationMessage;
...
pub(crate) async fn summarize_session_if_needed(state: &AppState, session_key: &str) {
    use synapse_core::application::services::conversation_service::WEB_SUMMARY_INTERVAL;
    ...
    let (summary_model, temperature, provider) = {
        ...
        let prov: std::sync::Arc<dyn crate::adapters::providers::Provider> =
            if let Some(ref provider_name) = sc.provider {
                let opts =
                    crate::adapters::providers::provider_runtime_options_from_config(&config_guard);
                let api_key = sc
                    .api_key_env
                    .as_deref()
                    .and_then(|env| std::env::var(env).ok());
                match crate::adapters::providers::create_provider_with_options(
                    provider_name,
                    api_key.as_deref(),
                    &opts,
                ) {
    ...
    let generator =
        crate::adapters::memory::summary_generator_adapter::ProviderSummaryGenerator::new(
            provider,
            summary_model,
            temperature,
        );

    match synapse_core::application::services::conversation_service::generate_session_summary(
        store.as_ref(),
        &generator,
        session_key,
        msg_count,
