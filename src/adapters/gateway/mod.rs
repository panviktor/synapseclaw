...
use crate::adapters::channels::{
    Channel, LinqChannel, NextcloudTalkChannel, SendMessage, WatiChannel, WhatsAppChannel,
};
use crate::adapters::cost::CostTracker;
use crate::adapters::providers::{self, ChatMessage, Provider};
use crate::adapters::tools;
use crate::adapters::tools::traits::ToolSpec;
...
fn whatsapp_memory_key(msg: &crate::adapters::channels::traits::ChannelMessage) -> String {
    format!("whatsapp_{}_{}", msg.sender, msg.id)
}

fn linq_memory_key(msg: &crate::adapters::channels::traits::ChannelMessage) -> String {
    format!("linq_{}_{}", msg.sender, msg.id)
}

fn wati_memory_key(msg: &crate::adapters::channels::traits::ChannelMessage) -> String {
    format!("wati_{}_{}", msg.sender, msg.id)
}

fn nextcloud_talk_memory_key(msg: &crate::adapters::channels::traits::ChannelMessage) -> String {
    format!("nextcloud_talk_{}_{}", msg.sender, msg.id)
}

fn sender_session_id(
    channel: &str,
    msg: &crate::adapters::channels::traits::ChannelMessage,
) -> String {
    match &msg.thread_ts {
...
pub struct AppState {
    ...
    pub agent_runner: Arc<dyn synapse_core::ports::agent_runner::AgentRunnerPort>,
    ...
    pub channel_session_backend:
        Option<Arc<dyn crate::adapters::channels::session_backend::SessionBackend>>,
    /// Phase 4.0: Channel adapter registry with long-lived cached instances
    pub channel_registry:
        Option<Arc<dyn synapse_core::ports::channel_registry::ChannelRegistryPort>>,
    /// Phase 4.0: Unified conversation/session store
    pub conversation_store:
        Option<Arc<dyn synapse_core::ports::conversation_store::ConversationStorePort>>,
    /// Phase 4.0: Unified run execution store
    pub run_store: Option<Arc<dyn synapse_core::ports::run_store::RunStorePort>>,
    /// Phase 4.1: Pipeline definition store (TOML loader)
    pub pipeline_store: Option<Arc<dyn synapse_core::ports::pipeline_store::PipelineStorePort>>,
    /// Phase 4.1: Pipeline step executor (IPC bridge)
    pub pipeline_executor:
        Option<Arc<dyn synapse_core::ports::pipeline_executor::PipelineExecutorPort>>,
    /// Phase 4.1: Message router for deterministic inbound routing
    pub message_router: Option<Arc<dyn synapse_core::ports::message_router::MessageRouterPort>>,
    /// Phase 4.1: Tool middleware chain
    pub tool_middleware: Option<
        Arc<synapse_core::application::services::tool_middleware_service::ToolMiddlewareChain>,
    >,
}
...
pub async fn run_gateway(
    ...
    port: u16,
    config: Config,
    outbound_tx: Option<synapse_core::bus::OutboundIntentSender>,
    channel_registry: Option<Arc<dyn synapse_core::ports::channel_registry::ChannelRegistryPort>>,
    shared_ipc_client: Option<Arc<crate::adapters::tools::agents_ipc::IpcClient>>,
    agent_runner: Arc<dyn synapse_core::ports::agent_runner::AgentRunnerPort>,
) -> Result<()> {
    ...
    // ── Hooks ──────────────────────────────────────────────────────
    let hooks: Option<std::sync::Arc<crate::adapters::hooks::HookRunner>> = if config.hooks.enabled
    {
        Some(std::sync::Arc::new(
            crate::adapters::hooks::HookRunner::new(),
        ))
    } else {
        None
    };
    ...
    // ── Tunnel ────────────────────────────────────────────────
    let tunnel = crate::adapters::tunnel::create_tunnel(&config.tunnel)?;
    ...
    crate::adapters::health::mark_component_ok("gateway");
    ...
    let broadcast_observer: Arc<dyn crate::adapters::observability::Observer> =
        Arc::new(sse::BroadcastObserver::new(
            crate::adapters::observability::create_observer(&config.observability),
            event_tx.clone(),
        ));
    ...
    let conversation_store: Option<
        Arc<dyn synapse_core::ports::conversation_store::ConversationStorePort>,
    > = chat_db.as_ref().map(|db| {
        Arc::new(
            crate::adapters::storage::conversation_store::ChatDbConversationStore::new(Arc::clone(
                db,
            )),
        ) as Arc<dyn synapse_core::ports::conversation_store::ConversationStorePort>
    });

    // ── Phase 4.0: RunStorePort (wraps ChatDb) ──
    let run_store: Option<Arc<dyn synapse_core::ports::run_store::RunStorePort>> =
        chat_db.as_ref().map(|db| {
            Arc::new(crate::adapters::storage::run_store::ChatDbRunStore::new(
                Arc::clone(db),
            )) as Arc<dyn synapse_core::ports::run_store::RunStorePort>
        });
    ...
    let mut state = AppState {
        ...
        channel_session_backend: if config.channels_config.session_persistence {
            match crate::adapters::channels::session_store::SessionStore::new(&config.workspace_dir)
            {
    ...
    if config.pipelines.enabled {
        ...
        let pipeline_store: Arc<dyn synapse_core::ports::pipeline_store::PipelineStorePort> =
            Arc::new(
                crate::adapters::pipeline::toml_loader::TomlPipelineLoader::new(&pipeline_dir),
            );
        ...
        let pipeline_executor: Option<
            Arc<dyn synapse_core::ports::pipeline_executor::PipelineExecutorPort>,
        > = if config.agents_ipc.enabled {
            if let Some(ref broker_token) = config.agents_ipc.broker_token {
                let ipc_client = if let Some(ref shared) = shared_ipc_client {
                    ...
                    let mut client = crate::adapters::tools::agents_ipc::IpcClient::new(
                        &broker_url,
                        broker_token,
                ...
                Some(Arc::new(
                    crate::adapters::pipeline::ipc_step_executor::IpcStepExecutor::new(ipc_client),
                )
                    as Arc<
                        dyn synapse_core::ports::pipeline_executor::PipelineExecutorPort,
                    >)
            } else {
        ...
        let message_router: Arc<dyn synapse_core::ports::message_router::MessageRouterPort> =
            Arc::new(
                crate::adapters::routing::rule_chain::TomlMessageRouter::load(
                    &routing_path,
                    &routing_fallback,
                ),
            );
        ...
        let mut middleware_chain =
            synapse_core::application::services::tool_middleware_service::ToolMiddlewareChain::new(
            );

        if config.pipelines.default_tool_rate_limit > 0 {
            middleware_chain.push(Box::new(
                crate::adapters::middleware::rate_limit::RateLimitMiddleware::with_default_limit(
        ...
        if !config.pipelines.approval_required_tools.is_empty() {
            let tools: std::collections::HashSet<String> = config
                ...
                .collect();
            middleware_chain.push(Box::new(
                crate::adapters::middleware::approval_gate::ApprovalGateMiddleware::new(tools),
            ));
        }
        ...
        if config.pipelines.hot_reload {
            match crate::adapters::pipeline::hot_reload::start_watcher(pipeline_dir, pipeline_store)
            {
                Ok(_handle) => {
        ...
        if let (Some(ref ps), Some(ref pe), Some(ref rs)) = (
            ...
            let ports =
                synapse_core::application::services::pipeline_service::PipelineRunnerPorts {
                    pipeline_store: ps.clone(),
                    run_store: rs.clone(),
                    executor: pe.clone(),
                };
            let report =
                synapse_core::application::use_cases::resume_pipeline::recover_all(&ports).await;
            if report.found > 0 {
                tracing::info!(
                    found = report.found,
...
async fn agent_inbox_processor(
    ...
    outbound_tx: Option<synapse_core::bus::OutboundIntentSender>,
    run_store: Option<Arc<dyn synapse_core::ports::run_store::RunStorePort>>,
    agent_runner: Arc<dyn synapse_core::ports::agent_runner::AgentRunnerPort>,
) {
    ...
    let ipc_client = crate::adapters::tools::agents_ipc::IpcClient::new(
    ...
    loop {
        ...
        for peer in &from_peers {
            ...
            let run_ctx = std::sync::Arc::new(synapse_core::domain::tool_audit::RunContext::new());

            // Phase 4.0: Track IPC run in RunStore
            let ipc_run_id = uuid::Uuid::new_v4().to_string();
            if let Some(ref store) = run_store {
                let run = synapse_core::domain::run::Run {
                    run_id: ipc_run_id.clone(),
                    conversation_key: Some(format!("ipc:{peer}")),
                    origin: synapse_core::domain::run::RunOrigin::Ipc,
                    state: synapse_core::domain::run::RunState::Running,
                ...
                Ok(last_text) => {
                    ...
                    if let Some(ref store) = run_store {
                        ...
                        let _ = store
                            .update_state(
                                &ipc_run_id,
                                synapse_core::domain::run::RunState::Completed,
                        ...
                        if let (Some(relay_ch), Some(relay_rcpt)) = (
                            ...
                            if let Some(tx) = &outbound_tx {
                                ...
                                if !relay_text.is_empty() {
                                    ...
                                    let intent =
                                        synapse_core::domain::channel::OutboundIntent::notify(
                ...
                Err(e) => {
                    ...
                    if let Some(ref store) = run_store {
                        ...
                        let _ = store
                            .update_state(
                                &ipc_run_id,
                                synapse_core::domain::run::RunState::Failed,
                                Some(chrono::Utc::now().timestamp() as u64),
                            )
                            .await;
                    }
...
async fn handle_health(State(state): State<AppState>) -> impl IntoResponse {
    let body = serde_json::json!({
        ...
        "runtime": crate::adapters::health::snapshot_json(),
    });
    Json(body)
}
    ...
    let body = {
        #[cfg(feature = "observability-prometheus")]
        {
            if let Some(prom) = state
                ...
                .as_ref()
                .as_any()
                .downcast_ref::<crate::adapters::observability::PrometheusObserver>(
            ) {
                prom.encode()
            } else {
...
async fn run_gateway_chat_simple(state: &AppState, message: &str) -> anyhow::Result<String> {
    ...
    let system_prompt = {
        ...
        crate::adapters::channels::build_system_prompt(
    ...
    let prepared =
        crate::adapters::multimodal::prepare_messages_for_provider(&messages, &multimodal_config)
            .await?;
...
async fn handle_webhook(
    ...
    state
        .observer
        .record_event(&crate::adapters::observability::ObserverEvent::AgentStart {
            provider: provider_label.clone(),
            model: model_label.clone(),
        });
    state
        .observer
        .record_event(&crate::adapters::observability::ObserverEvent::LlmRequest {
            provider: provider_label.clone(),
            model: model_label.clone(),
            messages_count: 1,
        });
    ...
    match run_gateway_chat_simple(&state, message).await {
        Ok(response) => {
            ...
            state.observer.record_event(
                &crate::adapters::observability::ObserverEvent::LlmResponse {
                    provider: provider_label.clone(),
                    model: model_label.clone(),
                    duration,
                    success: true,
            ...
            state.observer.record_metric(
                &crate::adapters::observability::traits::ObserverMetric::RequestLatency(duration),
            );
            state
                .observer
                .record_event(&crate::adapters::observability::ObserverEvent::AgentEnd {
        ...
        Err(e) => {
            ...
            state
                .observer
                .record_event(&crate::adapters::observability::ObserverEvent::Error {
...
async fn handle_linq_webhook(
    ...
    if let Some(ref signing_secret) = state.linq_signing_secret {
        ...
        if !crate::adapters::channels::linq::verify_linq_signature(
            signing_secret,
            &body_str,
            timestamp,
            signature,
...
async fn handle_nextcloud_talk_webhook(
    ...
    if let Some(ref webhook_secret) = state.nextcloud_talk_webhook_secret {
        ...
        if !crate::adapters::channels::nextcloud_talk::verify_nextcloud_talk_signature(
            webhook_secret,
            random,
            &body_str,
            signature,
...
mod tests {
    use super::*;
    use crate::adapters::channels::traits::ChannelMessage;
    ...
    use synapse_core::domain::memory::{MemoryCategory, MemoryEntry};
    use synapse_core::ports::memory_backend::Memory;

    struct NoopRunner;
    #[async_trait]
    impl synapse_core::ports::agent_runner::AgentRunnerPort for NoopRunner {
        async fn run(
            &self,
            _: Option<String>,
            _: Option<String>,
            ...
            _: Option<Vec<String>>,
            _: Option<Arc<synapse_core::domain::tool_audit::RunContext>>,
        ) -> anyhow::Result<String> {
            Ok(String::new())
        }
    ...
    fn test_agent_runner() -> Arc<dyn synapse_core::ports::agent_runner::AgentRunnerPort> {
        Arc::new(NoopRunner)
    }
    ...
    async fn metrics_endpoint_returns_hint_when_prometheus_is_disabled() {
        let state = AppState {
            ...
            wati: None,
            observer: Arc::new(crate::adapters::observability::NoopObserver),
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        let prom = Arc::new(crate::adapters::observability::PrometheusObserver::new());
        crate::adapters::observability::Observer::record_event(
            prom.as_ref(),
            &crate::adapters::observability::ObserverEvent::HeartbeatTick,
        );

        let observer: Arc<dyn crate::adapters::observability::Observer> = prom;
        let state = AppState {
            config: Arc::new(Mutex::new(Config::default())),
            provider: Arc::new(MockProvider::default()),
            model: "test-model".into(),
