...
pub async fn handle_api_status(
    ...
    let health = crate::adapters::health::snapshot();
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.lock().clone();
    match crate::adapters::cron::list_jobs(&config) {
        Ok(jobs) => {
...
pub async fn handle_api_cron_add(
    ...
    let config = state.config.lock().clone();
    let schedule = crate::adapters::cron::Schedule::Cron {
        expr: body.schedule,
        tz: None,
    };

    match crate::adapters::cron::add_shell_job_with_approval(
        &config,
        body.name,
        schedule,
        &body.command,
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = crate::adapters::cron::get_job(&config, &id) {
        return (
    ...
    match crate::adapters::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.lock().clone();
    match crate::adapters::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.lock().clone();
    let entries = crate::adapters::integrations::registry::all_integrations();
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        ...
        let enabled = matches!(
            status,
            crate::adapters::integrations::IntegrationStatus::Active
        );
        settings.insert(
...
pub async fn handle_api_doctor(
    ...
    let config = state.config.lock().clone();
    let results = crate::adapters::doctor::diagnose(&config);

    let ok_count = results
        .iter()
        .filter(|r| r.severity == crate::adapters::doctor::Severity::Ok)
        .count();
    let warn_count = results
        .iter()
        .filter(|r| r.severity == crate::adapters::doctor::Severity::Warn)
        .count();
    let error_count = results
        .iter()
        .filter(|r| r.severity == crate::adapters::doctor::Severity::Error)
        .count();
...
pub async fn handle_api_memory_list(
    ...
    if let Some(ref query) = params.query {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => synapse_core::domain::memory::MemoryCategory::Core,
            "daily" => synapse_core::domain::memory::MemoryCategory::Daily,
            "conversation" => synapse_core::domain::memory::MemoryCategory::Conversation,
            other => synapse_core::domain::memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(synapse_core::domain::memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = crate::adapters::tools::cli_discovery::discover_cli_tools(&[], &[]);
...
pub async fn handle_api_health(
    ...
    let snapshot = crate::adapters::health::snapshot();
    ...
    for name in &channels {
        ...
        if !caps.is_empty() {
            let cap_names: Vec<&str> = caps
                .iter()
                .map(|c| match c {
                    synapse_core::domain::channel::ChannelCapability::SendText => "SendText",
                    synapse_core::domain::channel::ChannelCapability::ReceiveText => "ReceiveText",
                    synapse_core::domain::channel::ChannelCapability::Threads => "Threads",
                    synapse_core::domain::channel::ChannelCapability::Reactions => "Reactions",
                    synapse_core::domain::channel::ChannelCapability::Typing => "Typing",
                    synapse_core::domain::channel::ChannelCapability::Attachments => "Attachments",
                    synapse_core::domain::channel::ChannelCapability::RichFormatting => {
                        "RichFormatting"
                    }
                    synapse_core::domain::channel::ChannelCapability::EditMessage => "EditMessage",
                    synapse_core::domain::channel::ChannelCapability::RuntimeCommands => {
                        "RuntimeCommands"
                    }
                    synapse_core::domain::channel::ChannelCapability::InterruptOnNewMessage => {
                        "InterruptOnNewMessage"
                    }
                    synapse_core::domain::channel::ChannelCapability::ToolContextDisplay => {
                        "ToolContextDisplay"
                    }
                })
                .collect();
...
pub async fn handle_api_channel_deliver(
    ...
    let mut intent = synapse_core::domain::channel::OutboundIntent::notify(
        channel,
        recipient,
        content.to_string(),
    );
...
pub async fn handle_api_activity(
    ...
    use crate::adapters::gateway::ipc::{ActivityEvent, TraceRef};
    ...
    // 2. Cron runs
    if let Ok(jobs) = crate::adapters::cron::list_jobs(&config) {
        for job in &jobs {
            if let Ok(runs) = crate::adapters::cron::list_runs(&config, &job.id, 10) {
                for run in &runs {
...
mod tests {
    ...
    fn hydrate_config_for_save_restores_masked_secrets_and_paths() {
        ...
        current.channels_config.email =
            Some(crate::adapters::channels::email_channel::EmailConfig {
                imap_host: "imap.example.com".to_string(),
                imap_port: 993,
                imap_folder: "INBOX".to_string(),
                smtp_host: "smtp.example.com".to_string(),
