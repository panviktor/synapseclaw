...
use crate::adapters::gateway::api::extract_bearer_token;
use axum::{
...
pub fn validate_send(
    ...
    synapse_core::domain::ipc::validate_send(
        from_agent,
        to_agent,
        kind,
        i32::from(from_level),
...
pub fn validate_state_set(trust_level: u8, agent_id: &str, key: &str) -> Result<(), IpcError> {
    synapse_core::domain::ipc::validate_state_write(i32::from(trust_level), agent_id, key).map_err(
        |acl_err| {
...
pub fn validate_state_get(trust_level: u8, key: &str) -> Result<(), IpcError> {
    synapse_core::domain::ipc::validate_state_read(i32::from(trust_level), key).map_err(|acl_err| {
        IpcError {
...
pub async fn handle_ipc_send(
    ...
    let resolved_to = synapse_core::application::services::ipc_service::resolve_recipient(
        &body.to,
    ...
    if synapse_core::application::services::ipc_service::session_limit_applies(
        i32::from(meta.trust_level),
        i32::from(to_level),
    ) {
        if let Some(ref sid) = body.session_id {
            ...
            if synapse_core::application::services::ipc_service::check_session_limit(
                count_usize,
                max_usize,
            ) {
                // Phase 4.0: escalation payload built by ipc_service
                let escalation_payload =
                    synapse_core::application::services::ipc_service::build_escalation_payload(
                        sid,
                        &meta.agent_id,
                        &resolved_to,
                        count_usize,
                ...
                let _ = db.insert_message(
                    &meta.agent_id,
                    &coordinator,
                    synapse_core::domain::ipc::ESCALATION_KIND,
                    &escalation_payload,
                    meta.trust_level,
                    Some(sid),
                    0,
    ...
    if synapse_core::application::services::ipc_service::should_complete_spawn(
        &body.kind,
        body.session_id.as_deref(),
    ) {
...
pub async fn handle_admin_activity(
    ...
    for agent in &agents {
        if !matches!(
            agent.status,
            crate::adapters::gateway::agent_registry::AgentStatus::Online
        ) {
            continue;
        }
...
mod tests {
    ...
    fn ipc_client_sign_send_body_adds_signature() {
        use crate::adapters::tools::agents_ipc::IpcClient;
...
pub async fn handle_pipeline_start(
    ...
    let ports = synapse_core::application::services::pipeline_service::PipelineRunnerPorts {
