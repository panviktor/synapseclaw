...
use crate::adapters::channels::session_backend::SessionBackend;
use crate::adapters::identity;
use crate::adapters::observability::traits::{ObserverEvent, ObserverMetric};
use crate::adapters::observability::{self, Observer};
...
type ChannelRouteSelection = synapse_core::ports::route_selection::RouteSelection;
...
use synapse_core::application::services::inbound_message_service::RuntimeCommand as ChannelRuntimeCommand;
...
impl InterruptOnNewMessageConfig {
    ...
    fn enabled_for_channel(
        self,
        caps: &[synapse_core::domain::channel::ChannelCapability],
    ) -> bool {
        synapse_core::application::services::inbound_message_service::should_interrupt_previous(
            self.enabled,
            caps,
        )
    }
...
struct ChannelRuntimeContext {
    ...
    activated_tools:
        Option<std::sync::Arc<std::sync::Mutex<crate::adapters::tools::ActivatedToolSet>>>,
    ...
    pipeline_store: Option<Arc<dyn synapse_core::ports::pipeline_store::PipelineStorePort>>,
    pipeline_executor:
        Option<Arc<dyn synapse_core::ports::pipeline_executor::PipelineExecutorPort>>,
    message_router: Option<Arc<dyn synapse_core::ports::message_router::MessageRouterPort>>,
}
...
fn conversation_history_key(msg: &traits::ChannelMessage) -> String {
    let envelope = crate::adapters::envelope_from_channel_message(msg);
    synapse_core::application::services::inbound_message_service::conversation_key(&envelope)
}
...
fn channel_delivery_instructions(
    channel_name: &str,
    registry: Option<&dyn synapse_core::ports::channel_registry::ChannelRegistryPort>,
) -> Option<String> {
...
fn parse_runtime_command(
    ...
    synapse_core::application::services::inbound_message_service::parse_runtime_command(
        content, caps,
    )
}
...
fn spawn_supervised_listener_with_health_interval(
    ...
    tokio::spawn(async move {
        ...
        loop {
            crate::adapters::health::mark_component_ok(&component);
            ...
            match result {
                Ok(()) => {
                    ...
                    crate::adapters::health::mark_component_error(
                        &component,
                ...
                Err(e) => {
                    ...
                    crate::adapters::health::mark_component_error(&component, e.to_string());
                }
            }

            crate::adapters::health::bump_component_restart(&component);
...
async fn handle_message_via_orchestrator(
    ...
    envelope: &synapse_core::domain::channel::InboundEnvelope,
    caps: &[synapse_core::domain::channel::ChannelCapability],
    original_msg: &traits::ChannelMessage,
) {
    use crate::adapters::inbound::{
        channel_output_adapter, conversation_history_adapter, route_selection_adapter,
        session_summary_adapter,
    };
    use crate::adapters::memory::memory_adapter;
    use crate::adapters::runtime::{agent_runtime_adapter, hooks_adapter};
    use synapse_core::application::use_cases::handle_inbound_message as uc;
    use synapse_core::ports::hooks::NoOpHooks;

    // ── Build ports from ChannelRuntimeContext ────────────────────
    let history_port: Arc<dyn synapse_core::ports::conversation_history::ConversationHistoryPort> =
        Arc::new(
    ...
    let route_port: Arc<dyn synapse_core::ports::route_selection::RouteSelectionPort> =
    ...
    let hooks_port: Arc<dyn synapse_core::ports::hooks::HooksPort> =
    ...
    let channel_output: Arc<dyn synapse_core::ports::channel_output::ChannelOutputPort> =
    ...
    let agent_runtime: Arc<dyn synapse_core::ports::agent_runtime::AgentRuntimePort> =
    ...
    let registry: Arc<dyn synapse_core::ports::channel_registry::ChannelRegistryPort> =
    ...
    let session_summary: Option<Arc<dyn synapse_core::ports::session_summary::SessionSummaryPort>> =
        ctx.session_store.as_ref().map(|store| {
            Arc::new(session_summary_adapter::SessionStoreAdapter::new(
                Arc::clone(store),
            )) as Arc<dyn synapse_core::ports::session_summary::SessionSummaryPort>
        });
    ...
    let memory_port: Option<Arc<dyn synapse_core::ports::memory::MemoryTiersPort>> =
    ...
    if let (Some(ref router), Some(ref store), Some(ref executor)) = (
        ...
        let routing_input = synapse_core::domain::routing::RoutingInput {
        ...
        if let Some(ref pipeline_name) = route_result.pipeline {
            if store.get(pipeline_name).await.is_some() {
                ...
                let run_store: Arc<dyn synapse_core::ports::run_store::RunStorePort> =
                    Arc::new(synapse_core::ports::run_store::NoOpRunStore);
                let pipeline_ports =
                ...
                let params =
                    synapse_core::application::services::pipeline_service::StartPipelineParams {
                ...
                let result = synapse_core::application::services::pipeline_service::run_pipeline(
                    &pipeline_ports,
                    params,
                )
                .await;
                ...
                let reply = match &result.state {
                    synapse_core::domain::pipeline_context::PipelineState::Completed => {
                        result
                            .data
                            .as_object()
                            .and_then(|obj| {
                    ...
                    synapse_core::domain::pipeline_context::PipelineState::Failed => {
    ...
    if let Some(ref _store) = ctx.session_store {
        let key = synapse_core::application::services::inbound_message_service::conversation_key(
            envelope,
        );
...
impl synapse_core::ports::channel_output::ChannelOutputPort for NullChannelOutput {
...
async fn format_command_effect(
    effect: &synapse_core::application::services::inbound_message_service::CommandEffect,
    ctx: &ChannelRuntimeContext,
    conversation_key: &str,
) -> String {
    use synapse_core::application::services::inbound_message_service::CommandEffect;
...
pub fn build_system_prompt_with_mode(
    ...
    if !skills.is_empty() {
        prompt.push_str(&crate::adapters::skills::skills_to_prompt_with_mode(
            skills,
            workspace_dir,
            skills_prompt_mode,
        ));
...
pub async fn start_channels(
    config: Config,
    shared_ipc_client: Option<std::sync::Arc<crate::adapters::tools::agents_ipc::IpcClient>>,
) -> Result<()> {
        ...
        match crate::adapters::tools::McpRegistry::connect_all(&config.mcp.servers).await {
            Ok(registry) => {
                ...
                if config.mcp.deferred_loading {
                    ...
                    built_tools.push(Box::new(crate::adapters::tools::ToolSearchTool::new(
                        deferred_set,
                        activated,
                    )));
                } else {
                    ...
                    for name in names {
                        if let Some(def) = registry.get_tool_def(&name).await {
                            ...
                            built_tools.push(Box::new(crate::adapters::tools::ArcToolRef(wrapper)));
                            registered += 1;
                        }
                    }
                    tracing::info!(
    ...
    let skills = crate::adapters::skills::load_skills_with_config(&workspace, &config);
    ...
    crate::adapters::health::mark_component_ok("channels");
    ...
    let (pipeline_store, pipeline_executor, message_router) = if config.pipelines.enabled {
        ...
        let store: Arc<dyn synapse_core::ports::pipeline_store::PipelineStorePort> = Arc::new(
        ...
        let fallback_agent = config
            ...
            .or_else(|| config.agents_ipc.agent_id.clone())
            .unwrap_or_else(|| config.agents_ipc.role.clone());
        let router: Option<Arc<dyn synapse_core::ports::message_router::MessageRouterPort>> = {
            ...
            let router = crate::adapters::routing::rule_chain::TomlMessageRouter::load(
                &routing_file,
                &fallback_agent,
            );
            ...
            Some(Arc::new(router)
                as Arc<
                    dyn synapse_core::ports::message_router::MessageRouterPort,
                >)
        };
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        hooks: if config.hooks.enabled {
            let mut runner = crate::adapters::hooks::HookRunner::new();
            if config.hooks.builtin.command_logger {
                runner.register(Box::new(
                    crate::adapters::hooks::builtin::CommandLoggerHook::new(),
                ));
            }
            if config.hooks.builtin.webhook_audit.enabled {
                runner.register(Box::new(
                    crate::adapters::hooks::builtin::WebhookAuditHook::new(
        ...
        channel_registry: Some(Arc::new(
            crate::adapters::channels::registry::CachedChannelRegistry::new(config.clone()),
        )),
        pipeline_store,
        pipeline_executor,
        message_router,
...
mod tests {
    ...
    use crate::adapters::providers::{ChatMessage, Provider};
    use std::collections::{HashMap, HashSet};
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    impl Memory for NoopMemory {
        ...
        async fn store(
            ...
            _key: &str,
            _content: &str,
            _category: synapse_core::domain::memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _limit: usize,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<synapse_core::domain::memory::MemoryEntry>> {
            Ok(Vec::new())
        }
        ...
        async fn get(
            &self,
            _key: &str,
        ) -> anyhow::Result<Option<synapse_core::domain::memory::MemoryEntry>> {
            Ok(None)
        }

        async fn list(
            &self,
            _category: Option<&synapse_core::domain::memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<synapse_core::domain::memory::MemoryEntry>> {
            Ok(Vec::new())
        }
    ...
    fn prompt_skills_include_instructions_and_tools() {
        let ws = make_workspace();
        let skills = vec![crate::adapters::skills::Skill {
            name: "code-review".into(),
            description: "Review code for bugs".into(),
            version: "1.0.0".into(),
            author: None,
            tags: vec![],
            tools: vec![crate::adapters::skills::SkillTool {
                name: "lint".into(),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        ...
        let result = std::panic::catch_unwind(|| {
            synapse_core::domain::util::truncate_with_ellipsis(msg, 80)
        });
        assert!(
            result.is_ok(),
    ...
    fn channel_notify_observer_truncates_utf8_arguments_safely() {
        ...
        observer.record_event(
            &crate::adapters::observability::traits::ObserverEvent::ToolCallStart {
...
This is an example JSON object for profile settings."#;
    ...
    async fn supervised_listener_marks_error_and_restarts_on_failures() {
        ...
        let snapshot = crate::adapters::health::snapshot_json();
    ...
    async fn supervised_listener_refreshes_health_while_running() {
        ...
        let first_last_ok = crate::adapters::health::snapshot_json()["components"][&component_name]
            ["last_ok"]
            .as_str()
            .unwrap_or("")
            .to_string();
        ...
        let second_last_ok = crate::adapters::health::snapshot_json()["components"]
