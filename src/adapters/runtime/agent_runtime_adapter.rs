...
//!
//! The port trait uses `synapse_core::domain::message::ChatMessage`, while
...
use crate::adapters::{from_core_message, to_core_message};
use anyhow::Result;
use async_trait::async_trait;
use std::sync::Arc;
use synapse_core::domain::message::ChatMessage as CoreChatMessage;
use synapse_core::ports::agent_runtime::{AgentRuntimePort, AgentTurnResult};
...
pub struct ChannelAgentRuntime {
    ...
    pub activated_tools: Option<Arc<std::sync::Mutex<crate::adapters::tools::ActivatedToolSet>>>,
...
impl AgentRuntimePort for ChannelAgentRuntime {
    async fn execute_turn(
        ...
        let fut = Box::pin(crate::agent::loop_::run_tool_call_loop(
            ...
            Some(&*self.approval_manager as &dyn synapse_core::ports::approval::ApprovalPort),
            &self.channel_name,
            &self.multimodal,
            max_iterations,
