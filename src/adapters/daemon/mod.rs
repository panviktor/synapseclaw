...
pub async fn run(
    ...
    host: String,
    port: u16,
    agent_runner: std::sync::Arc<dyn synapse_core::ports::agent_runner::AgentRunnerPort>,
) -> Result<()> {
    ...
    crate::adapters::health::mark_component_ok("daemon");

    if config.heartbeat.enabled {
        let _ = crate::adapters::heartbeat::engine::HeartbeatEngine::ensure_heartbeat_file(
    ...
    let shared_ipc_client: Option<std::sync::Arc<crate::adapters::tools::agents_ipc::IpcClient>> =
    ...
    let channel_registry: std::sync::Arc<
        dyn synapse_core::ports::channel_registry::ChannelRegistryPort,
    > = std::sync::Arc::new(
    ...
    let delivery_service = std::sync::Arc::new(
        synapse_core::application::services::delivery_service::DeliveryService::new(
    ...
    {
        let (tx, rx) = synapse_core::bus::outbound_intent_bus();
...

    {
        ...
        handles.push(spawn_component_supervisor(
            ...
            move || {
                ...
                async move {
                    Box::pin(crate::adapters::gateway::run_gateway(
        ...
        if has_supervised_channels(&config) {
            ...
            handles.push(spawn_component_supervisor(
                ...
                move || {
                    ...
                    async move {
                        Box::pin(crate::adapters::channels::start_channels(cfg, ipc)).await
                    }
                },
            ));
        } else {
    ...
    if config.cron.enabled {
        ...
        handles.push(spawn_component_supervisor(
            ...
            move || {
                ...
                async move { Box::pin(crate::adapters::cron::scheduler::run(cfg, ds, ar)).await }
            },
        ));
    } else {
        crate::adapters::health::mark_component_ok("scheduler");
    ...
    crate::adapters::health::mark_component_error("daemon", "shutdown requested");
...
fn spawn_state_writer(config: Config) -> JoinHandle<()> {
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            let mut json = crate::adapters::health::snapshot_json();
    ...
    Fut: Future<Output = Result<()>> + Send + 'static,
{
    tokio::spawn(async move {
        ...
        loop {
            crate::adapters::health::mark_component_ok(name);
            match run_component().await {
                ...
                Err(e) => {
                    crate::adapters::health::mark_component_error(name, e.to_string());
                    tracing::error!("Daemon component '{name}' failed: {e}");
                }
            }

            crate::adapters::health::bump_component_restart(name);
...
pub(crate) fn cron_delivery_config_from(
    delivery: &crate::adapters::cron::DeliveryConfig,
) -> CronDeliveryConfig {
    CronDeliveryConfig {
...
async fn run_heartbeat_worker(
    ...
    delivery_service: std::sync::Arc<
        synapse_core::application::services::delivery_service::DeliveryService,
    >,
    agent_runner: std::sync::Arc<dyn synapse_core::ports::agent_runner::AgentRunnerPort>,
) -> Result<()> {
    use crate::adapters::heartbeat::engine::{
        compute_adaptive_interval, HeartbeatEngine, HeartbeatTask, TaskPriority, TaskStatus,
    };
    use std::sync::Arc;

    let observer: std::sync::Arc<dyn crate::adapters::observability::Observer> =
        std::sync::Arc::from(crate::adapters::observability::create_observer(
                ...
                .await
            {
                Ok(response) => {
                    ...
                    if indices.is_empty() {
                        ...
                        crate::adapters::health::mark_component_ok("heartbeat");
                        #[allow(clippy::cast_precision_loss)]
                        let elapsed = tick_start.elapsed().as_millis() as f64;
                        metrics.lock().record_success(elapsed);
                        continue;
        ...
        for task in &tasks_to_run {
                ...
                .await
            {
                Ok(output) => {
                    ...
                    let _ = crate::adapters::heartbeat::store::record_run(
                        &config.workspace_dir,
                        &task.text,
                ...
                Err(e) => {
                    ...
                    crate::adapters::health::mark_component_error("heartbeat", e.to_string());
...
async fn outbound_intent_relay(
    registry: std::sync::Arc<dyn synapse_core::ports::channel_registry::ChannelRegistryPort>,
    mut rx: synapse_core::bus::OutboundIntentReceiver,
) {
...
mod tests {
    ...
    // synapse_core::application::services::delivery_service::tests.
