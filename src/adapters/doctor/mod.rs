...
fn doctor_model_targets(provider_override: Option<&str>) -> Vec<String> {
    ...
    crate::adapters::providers::list_providers()
        .into_iter()
...
pub async fn run_models(
    ...
    for provider_name in &targets {
        ...
        match crate::adapters::onboard::run_models_refresh(config, Some(provider_name), !use_cache)
            .await
        {
            Ok(()) => {
                ok_count += 1;
                println!("    ✅ model catalog check passed");
                let models_count = crate::adapters::onboard::wizard::cached_model_catalog_stats(
                    config,
                    provider_name,
                )
                .await?
...
pub fn run_traces(
    ...
    let path = crate::adapters::observability::runtime_trace::resolve_trace_path(
        &config.observability,
        &config.workspace_dir,
    );

    if let Some(target_id) = id.map(str::trim).filter(|value| !value.is_empty()) {
        match crate::adapters::observability::runtime_trace::find_event_by_id(&path, target_id)? {
            Some(event) => {
    ...
    let events = crate::adapters::observability::runtime_trace::load_events(
        &path,
        safe_limit,
        event_filter,
        contains,
...
fn provider_validation_error(name: &str) -> Option<String> {
    match crate::adapters::providers::create_provider(name, None) {
        Ok(_) => None,
        Err(err) => Some(
            err.to_string()
                .lines()
...
fn check_daemon_state(config: &Config, items: &mut Vec<DiagItem>) {
    let cat = "daemon";
    let state_file = crate::adapters::daemon::state_file_path(config);
...
fn check_cli_tools(items: &mut Vec<DiagItem>) {
    ...
    let discovered = crate::adapters::tools::cli_discovery::discover_cli_tools(&[], &[]);
