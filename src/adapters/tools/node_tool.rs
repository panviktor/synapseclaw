//! Wraps a node capability as a synapseclaw [`Tool`] so it can be dispatched
...
use crate::adapters::gateway::nodes::{NodeInvocation, NodeRegistry};
use crate::adapters::tools::traits::{Tool, ToolResult};

/// Default timeout for node invocations (30 seconds).
const NODE_INVOKE_TIMEOUT_SECS: u64 = 30;

/// A synapseclaw [`Tool`] backed by a node capability.
///
...
mod tests {
    use super::*;
    use crate::adapters::gateway::nodes::{NodeCapability, NodeInfo, NodeRegistry};
    ...
    async fn node_tool_execute_success() {
        ...
        tokio::spawn(async move {
            if let Some(invocation) = invoke_rx.recv().await {
                let _ = invocation.response_tx.send(
                    crate::adapters::gateway::nodes::NodeInvocationResult {
                        success: true,
