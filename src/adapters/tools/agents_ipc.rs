...
pub struct IpcClient {
    ...
    identity: Option<synapse_security::identity::AgentIdentity>,
...
impl IpcClient {
    ...
    pub fn new(broker_url: &str, bearer_token: &str, timeout_secs: u64) -> Self {
        ...
        let builder =
            synapse_config::schema::apply_runtime_proxy_to_builder(builder, "tool.agents_ipc");
    ...
    pub fn with_identity(
        mut self,
        identity: synapse_security::identity::AgentIdentity,
        agent_id: String,
    ) -> Self {
...
pub struct AgentsSpawnTool {
    config: Arc<synapse_config::schema::Config>,
    security: Arc<synapse_core::domain::security_policy::SecurityPolicy>,
...
impl AgentsSpawnTool {
    ...
    fn spawn_legacy(
        ...
        let schedule = crate::adapters::cron::Schedule::At { at: run_at };
        ...
        match crate::adapters::cron::add_agent_job(
            &self.config,
            Some(job_name.clone()),
            schedule,
            &spawn_prompt,
            crate::adapters::cron::SessionTarget::Isolated,
            model,
            None,
            true,
        ) {
    ...
    async fn spawn_broker_backed(
        ...
        let boundary = synapse_security::execution::execution_boundary(child_level);
        let sandbox = synapse_security::detect::create_sandbox(&self.config.security);
        if let Err(e) =
            synapse_security::execution::require_sandbox(child_level, &boundary, sandbox.as_ref())
        {
            return Ok(ToolResult {
                success: false,
                output: String::new(),
        ...
        let resolved_workload = if let Some(ref workload_name) = workload {
            if let Some(profile) = self.config.agents_ipc.workload_profiles.get(workload_name) {
                match synapse_security::execution::apply_workload(&boundary, profile) {
        ...
        let autonomy_str = match boundary.autonomy {
            synapse_security::execution::BoundaryAutonomy::Full => "full",
            synapse_security::execution::BoundaryAutonomy::Supervised => "supervised",
            synapse_security::execution::BoundaryAutonomy::ReadOnly => "read_only",
        };
        ...
        let job = crate::adapters::cron::add_agent_job_full(
            &self.config,
            Some(job_name.clone()),
            schedule,
            &spawn_prompt,
            ...
            None,
            true,
            crate::adapters::cron::ExecutionMode::Subprocess,
            env_overlay,
        )
...
mod tests {
    ...
    fn agents_spawn_tool_spec() {
        let config = Arc::new(synapse_config::schema::Config::default());
        let security = Arc::new(synapse_core::domain::security_policy::SecurityPolicy::default());
    ...
    use crate::adapters::gateway::ipc::{
    ...
    use crate::adapters::gateway::AppState;
    ...
    struct TestProvider;
    #[async_trait]
    impl crate::adapters::providers::traits::Provider for TestProvider {
    ...
    struct TestMemory;
    #[async_trait]
    impl synapse_core::ports::memory_backend::Memory for TestMemory {
        fn name(&self) -> &str {
            "test"
        }
        async fn store(
    ...
    fn test_app_state(db: Arc<IpcDb>, token_hash: &str) -> AppState {
        ...
        let pairing = std::sync::Arc::new(synapse_security::PairingGuard::with_metadata(
            true,
            &[token_hash.to_string()],
            &metadata,
        ));
        ...
        AppState {
            ...
            rate_limiter: std::sync::Arc::new(crate::adapters::gateway::GatewayRateLimiter::new(
                100, 100, 100,
            )),
            idempotency_store: std::sync::Arc::new(
                crate::adapters::gateway::IdempotencyStore::new(
            ...
            wati: None,
            observer: std::sync::Arc::new(crate::adapters::observability::NoopObserver),
            ...
            node_registry: std::sync::Arc::new(crate::adapters::gateway::nodes::NodeRegistry::new(
                16,
            )),
            agent_registry: std::sync::Arc::new(
                crate::adapters::gateway::agent_registry::AgentRegistry::new(),
            ),
            agent_runner: {
                struct NR;
                #[async_trait::async_trait]
                impl synapse_core::ports::agent_runner::AgentRunnerPort for NR {
                    async fn run(
                        &self,
                        _: Option<String>,
                        _: Option<String>,
                        ...
                        _: Option<Vec<String>>,
                        _: Option<std::sync::Arc<synapse_core::domain::tool_audit::RunContext>>,
                    ) -> anyhow::Result<String> {
                        Ok(String::new())
                    }
            ...
            provisioning_state: std::sync::Arc::new(
                crate::adapters::gateway::provisioning::ProvisioningState::new(),
            ),
