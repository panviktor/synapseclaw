//! Wraps a discovered MCP tool as a synapseclaw [`Tool`] so it is dispatched
...
use crate::adapters::tools::mcp_client::McpRegistry;
use crate::adapters::tools::mcp_protocol::McpToolDef;
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // SynapseClaw's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // SynapseClaw's security model injects `approved: bool` into built-in tool args.
