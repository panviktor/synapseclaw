//! Flash SynapseClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash SynapseClaw Nucleo firmware. Builds from firmware/nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (synapseclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from synapseclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building SynapseClaw Nucleo firmware...");
    ...
    println!("SynapseClaw Nucleo firmware flashed successfully.");
