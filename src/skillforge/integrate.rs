//! Integrator — generates SynapseClaw-standard SKILL.toml + SKILL.md from scout results.
...
```toml
# Add to your SynapseClaw config:
[skills.{name}]
enabled = true
```
...
mod tests {
    ...
    async fn integrate_creates_files() {
        let tmp = std::env::temp_dir().join("synapseclaw-test-integrate");
