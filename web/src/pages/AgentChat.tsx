...
import { getStatus, getAgentStatus, putSummaryModel, getAgents, deleteChannelSession, type AgentEntry } from '@/lib/api';
...
export default function AgentChat() {
  ...
  // Channel sessions are now included in the unified sessions.list response.
  // The `kind` field distinguishes web vs channel sessions.
  ...
  // ── Restore activeSession from localStorage if URL has none ──────
  useEffect(() => {
    if (!activeSession) {
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {sessions.find((s) => s.key === activeSession)?.kind === 'channel' ? (
          <div className="border-t border-[var(--border-default)] px-4 py-3 bg-[var(--bg-card)] text-center">
            <p className="text-xs text-[var(--text-muted)]">
              Read-only conversation from <strong>{sessions.find((s) => s.key === activeSession)?.channel ?? 'channel'}</strong>. Reply through the channel client.
            </p>
          </div>
        ) : <div
