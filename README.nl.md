...
<p align="center">
  SynapseClaw is het <strong>runtime besturingssysteem</strong> voor agent workflows — een infrastructuur die modellen, tools, geheugen en uitvoering abstraheert om agenten één keer te bouwen en overal uit te voeren.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritiek_  | **We zijn niet gelieerd** met `openagen/synapseclaw` of `synapseclaw.org`. Het domein `synapseclaw.org` wijst momenteel naar de fork `openagen/synapseclaw`, en dit domein/repository imiteert onze officiële website/project.                                                                                                                                                                                 | Vertrouw geen informatie, binaire bestanden, fondsenwerving of aankondigingen van deze bronnen. Gebruik alleen [deze repository](https://github.com/panviktor/synapseclaw) en onze geverifieerde sociale media accounts.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Belangrijk_ | Onze officiële website is nu online: [synapseclaw.dev](https://synapseclaw.dev). Bedankt voor je geduld tijdens het wachten. We detecteren nog steeds imitatiepogingen: neem niet deel aan enige investering/fondsenwerving activiteit in naam van SynapseClaw als deze niet via onze officiële kanalen wordt gepubliceerd.                                                                                                                   | Gebruik [deze repository](https://github.com/panviktor/synapseclaw) als de enige bron van waarheid. Volg [X (@synapseclaw)](https://x.com/synapseclaw?s=21), [Telegram (@synapseclaw)](https://t.me/synapseclaw), [Facebook (groep)](https://www.facebook.com/groups/synapseclaw), [Reddit (r/synapseclaw)](https://www.reddit.com/r/synapseclaw/), en [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) voor officiële updates. |
...
### Waarom teams kiezen voor SynapseClaw
...
## Benchmark Snapshot (SynapseClaw vs OpenClaw, Reproduceerbaar)
...
> Opmerkingen: SynapseClaw resultaten worden gemeten op productie builds met `/usr/bin/time -l`. OpenClaw vereist de Node.js runtime (typisch ~390 MB extra geheugen overhead), terwijl NanoBot de Python runtime vereist. PicoClaw en SynapseClaw zijn statische binaries. De bovenstaande RAM cijfers zijn runtime geheugen; build-time compilatievereisten zijn hoger.

<p align="center">
  <img src="zero-claw.jpeg" alt="SynapseClaw vs OpenClaw Vergelijking" width="800" />
</p>
...
- `synapseclaw --help`: werkelijke tijd ongeveer `0.02s`, piek geheugengebruik ~`3.9 MB`
- `synapseclaw status`: werkelijke tijd ongeveer `0.01s`, piek geheugengebruik ~`4.1 MB`
...
Het `bootstrap.sh` script installeert Rust, kloont SynapseClaw, compileert het, en stelt je initiële ontwikkelomgeving in:
...
2. De SynapseClaw repository klonen
3. SynapseClaw compileren in release modus
4. `synapseclaw` installeren in `~/.cargo/bin/`
5. De standaard workspace structuur maken in `~/.synapseclaw/workspace/`
6. Een initiële configuratie `~/.synapseclaw/workspace/config.toml` genereren

Na de bootstrap, herlaad je shell of voer `source ~/.cargo/env` uit om het `synapseclaw` commando globaal te gebruiken.
...
1. Configureer je AI providers in `~/.synapseclaw/workspace/config.toml`
2. Bekijk de [configuratie referentie](docs/config-reference.md) voor geavanceerde opties
3. Start de agent: `synapseclaw agent start`
4. Test via je voorkeurskanaal (zie [kanalen referentie](docs/channels-reference.md))

## Configuratie

Bewerk `~/.synapseclaw/workspace/config.toml` om providers, kanalen en systeemgedrag te configureren.
...
SynapseClaw ondersteunt twee code uitvoeringsbackends:
...
```bash
# Workspace beheer
synapseclaw init                # Initialiseert een nieuwe workspace
synapseclaw status              # Toont daemon/agent status
synapseclaw config validate     # Verifieert config.toml syntax en waarden

# Daemon beheer
synapseclaw daemon start        # Start de daemon in de achtergrond
synapseclaw daemon stop         # Stopt de draaiende daemon
synapseclaw daemon restart      # Herstart de daemon (config herladen)
synapseclaw daemon logs         # Toont daemon logs

# Agent beheer
synapseclaw agent start         # Start de agent (vereist draaiende daemon)
synapseclaw agent stop          # Stopt de agent
synapseclaw agent restart       # Herstart de agent (config herladen)

# Pairing operaties
synapseclaw pairing init        # Genereert een nieuw pairing geheim
synapseclaw pairing rotate      # Roteert het bestaande pairing geheim

# Tunneling (voor publieke blootstelling)
synapseclaw tunnel start        # Start een tunnel naar de lokale daemon
synapseclaw tunnel stop         # Stopt de actieve tunnel

# Diagnostiek
synapseclaw doctor              # Voert systeem gezondheidscontroles uit
synapseclaw version             # Toont versie en build informatie
```
...
## SynapseClaw Ondersteunen

Als SynapseClaw je werk helpt en je de doorlopende ontwikkeling wilt ondersteunen, kun je hier doneren:
...
**Dit is de enige officiële SynapseClaw repository:**

> <https://github.com/panviktor/synapseclaw>

Elke andere repository, organisatie, domein of pakket dat beweert "SynapseClaw" te zijn of affiniteit met SynapseClaw Labs suggereert is **niet-geautoriseerd en niet gelieerd aan dit project**. Bekende niet-geautoriseerde forks worden vermeld in [TRADEMARK.md](TRADEMARK.md).

Als je imitatie of handelsmerk misbruik tegenkomt, [open dan een issue](https://github.com/panviktor/synapseclaw/issues).

---

## Licentie

SynapseClaw is dubbel gelicentieerd voor maximale openheid en bijdrager bescherming:
...
De naam **SynapseClaw** en het logo zijn geregistreerde handelsmerken van SynapseClaw Labs. Deze licentie verleent geen toestemming om ze te gebruiken om goedkeuring of affiniteit te impliceren. Zie [TRADEMARK.md](TRADEMARK.md) voor toegestane en verboden gebruiksmogelijkheden.
...
- Nieuwe `Skill` → `~/.synapseclaw/workspace/skills/<n>/`

---

**SynapseClaw** — Nul overhead. Nul compromis. Implementeer overal. Wissel alles. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#panviktor/synapseclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Sterren Geschiedenis Grafiek" src="https://api.star-history.com/svg?repos=panviktor/synapseclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
