...
<h1 align="center">SynapseClaw 🦀（日本語）</h1>
...
<p align="center">
  <a href="https://raw.githubusercontent.com/panviktor/synapseclaw/master/install.sh">ワンクリック導入</a> |
...
| 日付 (UTC) | レベル | お知らせ | 対応 |
|---|---|---|---|
| 2026-02-19 | _緊急_ | 私たちは `openagen/synapseclaw` および `synapseclaw.org` とは**一切関係ありません**。`synapseclaw.org` は現在 `openagen/synapseclaw` の fork を指しており、そのドメイン/リポジトリは当プロジェクトの公式サイト・公式プロジェクトを装っています。 | これらの情報源による案内、バイナリ、資金調達情報、公式発表は信頼しないでください。必ず[本リポジトリ](https://github.com/panviktor/synapseclaw)と認証済み公式SNSのみを参照してください。 |
| 2026-02-21 | _重要_ | 公式サイトを公開しました: [synapseclaw.dev](https://synapseclaw.dev)。公開までお待ちいただきありがとうございました。引き続きなりすましの試みを確認しているため、SynapseClaw 名義の投資・資金調達などの案内は、公式チャネルで確認できない限り参加しないでください。 | 情報は[本リポジトリ](https://github.com/panviktor/synapseclaw)を最優先で確認し、[X（@synapseclaw）](https://x.com/synapseclaw?s=21)、[Telegram（@synapseclaw）](https://t.me/synapseclaw)、[Facebook（グループ）](https://www.facebook.com/groups/synapseclaw)、[Reddit（r/synapseclaw）](https://www.reddit.com/r/synapseclaw/) と [小紅書アカウント](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) で公式更新を確認してください。 |
...
SynapseClaw は、高速・省リソース・高拡張性を重視した自律エージェント実行基盤です。SynapseClawはエージェントワークフローのための**ランタイムオペレーティングシステム**です — モデル、ツール、メモリ、実行を抽象化し、エージェントを一度構築すればどこでも実行できるインフラストラクチャです。
...
## ベンチマークスナップショット（SynapseClaw vs OpenClaw、再現可能）
...
> 注記: SynapseClaw の結果は release ビルドを `/usr/bin/time -l` で計測したものです。OpenClaw は Node.js ランタイムが必要で、ランタイム由来だけで通常は約390MBの追加メモリを要します。NanoBot は Python ランタイムが必要です。PicoClaw と SynapseClaw は静的バイナリです。
...
- Release バイナリ: `8.8M`
- `synapseclaw --help`: 約 `0.02s`、ピークメモリ 約 `3.9MB`
- `synapseclaw status`: 約 `0.01s`、ピークメモリ 約 `4.1MB`
...
SynapseClaw はサブスクリプションベースのネイティブ認証プロファイルをサポートしています（マルチアカウント対応、保存時暗号化）。

- 保存先: `~/.synapseclaw/auth-profiles.json`
- 暗号化キー: `~/.synapseclaw/.secret_key`
