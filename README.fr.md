...
<p align="center">
  ...
  <a href="https://raw.githubusercontent.com/panviktor/synapseclaw/master/install.sh">Configuration en un clic</a> |
...
<p align="center">
  SynapseClaw est le <strong>système d'exploitation runtime</strong> pour les workflows agentiques — une infrastructure qui abstrait les modèles, outils, mémoire et exécution pour construire des agents une fois et les exécuter partout.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critique_  | Nous ne sommes **pas affiliés** à `openagen/synapseclaw` ou `synapseclaw.org`. Le domaine `synapseclaw.org` pointe actuellement vers le fork `openagen/synapseclaw`, et ce domaine/dépôt usurpe l'identité de notre site web/projet officiel.                                                                                                                                                                                 | Ne faites pas confiance aux informations, binaires, levées de fonds ou annonces provenant de ces sources. Utilisez uniquement [ce dépôt](https://github.com/panviktor/synapseclaw) et nos comptes sociaux vérifiés.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Important_ | Notre site officiel est désormais en ligne : [synapseclaw.dev](https://synapseclaw.dev). Merci pour votre patience pendant cette attente. Nous constatons toujours des tentatives d'usurpation : ne participez à aucune activité d'investissement/financement au nom de SynapseClaw si elle n'est pas publiée via nos canaux officiels.                                                                                                                   | Utilisez [ce dépôt](https://github.com/panviktor/synapseclaw) comme source unique de vérité. Suivez [X (@synapseclaw)](https://x.com/synapseclaw?s=21), [Facebook (groupe)](https://www.facebook.com/groups/synapseclaw), et [Reddit (r/synapseclaw)](https://www.reddit.com/r/synapseclaw/) pour les mises à jour officielles. |
...
### Pourquoi les équipes choisissent SynapseClaw
...
## Instantané de Benchmark (SynapseClaw vs OpenClaw, Reproductible)
...
> Notes : Les résultats SynapseClaw sont mesurés sur des builds de production utilisant `/usr/bin/time -l`. OpenClaw nécessite le runtime Node.js (typiquement ~390 Mo de surcharge mémoire supplémentaire), tandis que NanoBot nécessite le runtime Python. PicoClaw et SynapseClaw sont des binaires statiques. Les chiffres RAM ci-dessus sont la mémoire runtime ; les exigences de compilation build-time sont plus élevées.

<p align="center">
  <img src="docs/assets/synapseclaw-comparison.jpeg" alt="Comparaison SynapseClaw vs OpenClaw" width="800" />
</p>
...
- `synapseclaw --help` : environ `0.02s` de temps réel, ~`3.9 Mo` d'empreinte mémoire maximale
- `synapseclaw status` : environ `0.01s` de temps réel, ~`4.1 Mo` d'empreinte mémoire maximale
...
Le script `install.sh` installe Rust, clone SynapseClaw, le compile, et configure votre environnement de développement initial :
...
2. Cloner le dépôt SynapseClaw
3. Compiler SynapseClaw en mode release
4. Installer `synapseclaw` dans `~/.cargo/bin/`
5. Créer la structure de workspace par défaut dans `~/.synapseclaw/workspace/`
6. Générer un fichier de configuration `~/.synapseclaw/workspace/config.toml` de démarrage

Après le bootstrap, relancez votre shell ou exécutez `source ~/.cargo/env` pour utiliser la commande `synapseclaw` globalement.
...
1. Configurez vos fournisseurs d'IA dans `~/.synapseclaw/workspace/config.toml`
2. Consultez la [référence de configuration](docs/reference/api/config-reference.md) pour les options avancées
3. Lancez l'agent : `synapseclaw agent start`
4. Testez via votre canal préféré (voir [référence des canaux](docs/reference/api/channels-reference.md))

## Configuration

Éditez `~/.synapseclaw/workspace/config.toml` pour configurer les fournisseurs, canaux et comportement du système.
...
SynapseClaw prend en charge deux backends d'exécution de code :
...
synapseclaw init                # Initialise un nouveau workspace
synapseclaw status              # Affiche l'état du daemon/agent
synapseclaw config validate     # Vérifie la syntaxe et les valeurs de config.toml

# Gestion du daemon
synapseclaw daemon start        # Démarre le daemon en arrière-plan
synapseclaw daemon stop         # Arrête le daemon en cours d'exécution
synapseclaw daemon restart      # Redémarre le daemon (rechargement de config)
synapseclaw daemon logs         # Affiche les journaux du daemon

# Gestion de l'agent
synapseclaw agent start         # Démarre l'agent (nécessite daemon en cours d'exécution)
synapseclaw agent stop          # Arrête l'agent
synapseclaw agent restart       # Redémarre l'agent (rechargement de config)

# Opérations de pairing
synapseclaw pairing init        # Génère un nouveau secret de pairing
synapseclaw pairing rotate      # Fait tourner le secret de pairing existant

# Tunneling (pour exposition publique)
synapseclaw tunnel start        # Démarre un tunnel vers le daemon local
synapseclaw tunnel stop         # Arrête le tunnel actif

# Diagnostic
synapseclaw doctor              # Exécute les vérifications de santé du système
synapseclaw version             # Affiche la version et les informations de build
```
...
## Soutenir SynapseClaw

Si SynapseClaw aide votre travail et que vous souhaitez soutenir le développement continu, vous pouvez faire un don ici :
...
**Ceci est le seul dépôt officiel SynapseClaw :**

> <https://github.com/panviktor/synapseclaw>

Tout autre dépôt, organisation, domaine ou package prétendant être "SynapseClaw" ou impliquant une affiliation avec SynapseClaw Labs est **non autorisé et non affilié à ce projet**. Les forks non autorisés connus seront listés dans [TRADEMARK.md](docs/maintainers/trademark.md).

Si vous rencontrez une usurpation d'identité ou une utilisation abusive de marque, veuillez [ouvrir une issue](https://github.com/panviktor/synapseclaw/issues).

---

## Licence

SynapseClaw est sous double licence pour une ouverture maximale et la protection des contributeurs :
...
Le nom **SynapseClaw** et le logo sont des marques déposées de SynapseClaw Labs. Cette licence n'accorde pas la permission de les utiliser pour impliquer une approbation ou une affiliation. Voir [TRADEMARK.md](docs/maintainers/trademark.md) pour les utilisations permises et interdites.
...
- Nouvelle `Skill` → `~/.synapseclaw/workspace/skills/<n>/`

---

**SynapseClaw** — Zéro surcharge. Zéro compromis. Déployez n'importe où. Échangez n'importe quoi. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#panviktor/synapseclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Graphique Historique des Étoiles" src="https://api.star-history.com/svg?repos=panviktor/synapseclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
