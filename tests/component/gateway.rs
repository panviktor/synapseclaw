...
//! verification) in isolation. The gateway module (`synapseclaw::gateway`) exposes
...
use synapseclaw::gateway::verify_whatsapp_signature;
...
fn gateway_body_limit_is_reasonable() {
    assert_eq!(
        synapseclaw::gateway::MAX_BODY_SIZE,
        65_536,
...
fn gateway_timeout_is_reasonable() {
    assert_eq!(
        synapseclaw::gateway::REQUEST_TIMEOUT_SECS,
        30,
...
fn gateway_rate_limit_window_is_60s() {
    assert_eq!(
        synapseclaw::gateway::RATE_LIMIT_WINDOW_SECS,
        60,
