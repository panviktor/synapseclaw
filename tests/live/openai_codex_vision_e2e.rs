...
use anyhow::Result;
use synapseclaw::providers::{ChatMessage, ChatRequest, ProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let provider = synapseclaw::providers::create_provider_with_options("openai-codex", None, &opts)?;
