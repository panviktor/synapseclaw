...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.synapseclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.synapseclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let synapseclaw_dir = PathBuf::from(home).join(".synapseclaw");
    let auth_profiles_path = synapseclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: synapseclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: synapseclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.synapseclaw/auth-profiles.json (with expired token)
    let provider = synapseclaw::providers::create_provider("gemini", None)?;
