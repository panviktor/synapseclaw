...
/// Validates that with NoopMemory (no enrichment), user messages are stored
/// raw in history and the provider receives them with only a timestamp prefix.
#[tokio::test]
async fn e2e_noop_memory_raw_user_messages() {
    ...
    // Provider message ends with original text
    let requests = recorded.lock().unwrap();
    ...
    assert!(
        ...
        "Noop memory should not prepend memory context text, got: {}",
        user_msg.content
    );

    // History stores raw user message (with timestamp but no enrichment)
    let history = agent.history();
    match &history[1] {
        ConversationMessage::Chat(c) => {
            ...
            assert!(
                !c.content.contains("[Memory context]"),
                "History should store raw user message, got: {}",
                c.content
            );
        }
