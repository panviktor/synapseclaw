#!/bin/bash
# SynapseClaw Telegram Integration Test Suite
...
if [ -f "target/release/synapseclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/synapseclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/synapseclaw 2>/dev/null || stat -c%s target/release/synapseclaw)
...
CONFIG_PATH="$HOME/.synapseclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'synapseclaw onboard' first"
    fi
else
    warn "No config file found - run 'synapseclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/synapseclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
