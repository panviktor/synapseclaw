...
This guide covers testing the Telegram channel integration for SynapseClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug synapseclaw channel start
    ```
...
6. **Health check timeout**
    ...
    ```bash
    time synapseclaw channel doctor
    ```
...
Solution: Check user allowlist
  ...
  4. Run: synapseclaw onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info synapseclaw daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time synapseclaw channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep synapseclaw`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/synapseclaw` |
...
```bash
# Maximum logging
RUST_LOG=trace synapseclaw channel start
...
# Validate config
cat ~/.synapseclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [SynapseClaw Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/panviktor/synapseclaw/issues)
