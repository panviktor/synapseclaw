...
<p align="center">
  Ang SynapseClaw ay ang <strong>runtime operating system</strong> para sa agent workflows — isang infrastructure na nag-a-abstract ng mga modelo, tools, memory, at execution upang bumuo ng mga agent nang isang beses at patakbuhin ang mga ito kahit saan.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritikal_  | **Hindi kami kaugnay** sa `openagen/synapseclaw` o `synapseclaw.org`. Ang domain na `synapseclaw.org` ay kasalukuyang tumuturo sa fork na `openagen/synapseclaw`, at ang domain/repository na ito ay nanggagaya sa aming opisyal na website/proyekto.                                                                                                                                                                                 | Huwag magtiwala sa impormasyon, binaries, fundraising, o mga anunsyo mula sa mga pinagmulang ito. Gamitin lamang [ang repository na ito](https://github.com/panviktor/synapseclaw) at aming mga verified social media accounts.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Mahalaga_ | Ang aming opisyal na website ay ngayon online: [synapseclaw.dev](https://synapseclaw.dev). Salamat sa iyong pasensya sa panahon ng paghihintay. Nakikita pa rin namin ang mga pagtatangka ng panliliko: huwag lumahok sa anumang investment/funding activity sa ngalan ng SynapseClaw kung hindi ito nai-publish sa pamamagitan ng aming mga opisyal na channel.                                                                                                                   | Gamitin [ang repository na ito](https://github.com/panviktor/synapseclaw) bilang nag-iisang source of truth. Sundan [X (@synapseclaw)](https://x.com/synapseclaw?s=21), [Telegram (@synapseclaw)](https://t.me/synapseclaw), [Facebook (grupo)](https://www.facebook.com/groups/synapseclaw), [Reddit (r/synapseclaw)](https://www.reddit.com/r/synapseclaw/), at [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) para sa mga opisyal na update. |
...
### Bakit pinipili ng mga team ang SynapseClaw
...
> Mga Tala: Ang mga resulta ng SynapseClaw ay sinusukat sa production builds gamit ang `/usr/bin/time -l`. Ang OpenClaw ay nangangailangan ng Node.js runtime (typically ~390 MB additional memory overhead), habang ang NanoBot ay nangangailangan ng Python runtime. Ang PicoClaw at SynapseClaw ay static binaries. Ang mga RAM figure sa itaas ay runtime memory; ang build-time compilation requirements ay mas mataas.

<p align="center">
  <img src="zero-claw.jpeg" alt="SynapseClaw vs OpenClaw Comparison" width="800" />
</p>
...
- `synapseclaw --help`: real time na humigit-kumulang `0.02s`, peak memory footprint ~`3.9 MB`
- `synapseclaw status`: real time na humigit-kumulang `0.01s`, peak memory footprint ~`4.1 MB`
...
Ang `bootstrap.sh` script ay nag-i-install ng Rust, nagi-clone ng SynapseClaw, nagi-compile, at nagse-set up ng iyong paunang development environment:

```bash
curl -fsSL https://raw.githubusercontent.com/panviktor/synapseclaw/master/bootstrap.sh | bash
```

Ito ay:

1. Mag-i-install ng Rust (kung wala)
2. Magi-clone ng SynapseClaw repository
3. Magi-compile ng SynapseClaw sa release mode
4. Mag-i-install ng `synapseclaw` sa `~/.cargo/bin/`
5. Gagawa ng default workspace structure sa `~/.synapseclaw/workspace/`
6. Gagawa ng paunang configuration file na `~/.synapseclaw/workspace/config.toml`

Pagkatapos ng bootstrap, i-reload ang iyong shell o patakbuhin ang `source ~/.cargo/env` para gamitin ang `synapseclaw` command globally.
...
1. I-configure ang iyong AI providers sa `~/.synapseclaw/workspace/config.toml`
2. Tingnan ang [configuration reference](docs/config-reference.md) para sa advanced options
3. Simulan ang agent: `synapseclaw agent start`
4. I-test sa pamamagitan ng iyong preferred channel (tingnan ang [channels reference](docs/channels-reference.md))

## Configuration

I-edit ang `~/.synapseclaw/workspace/config.toml` para i-configure ang providers, channels, at system behavior.
...
Sinusuportahan ng SynapseClaw ang dalawang code execution backends:
...
synapseclaw init                # Nag-initialize ng bagong workspace
synapseclaw status              # Nagpapakita ng daemon/agent status
synapseclaw config validate     # Nag-verify ng config.toml syntax at values

# Daemon management
synapseclaw daemon start        # Nagse-start ng daemon sa background
synapseclaw daemon stop         # Naghihinto sa running daemon
synapseclaw daemon restart      # Nagre-restart ng daemon (config reload)
synapseclaw daemon logs         # Nagpapakita ng daemon logs

# Agent management
synapseclaw agent start         # Nagse-start ng agent (nangangailangan ng running daemon)
synapseclaw agent stop          # Naghihinto sa agent
synapseclaw agent restart       # Nagre-restart ng agent (config reload)

# Pairing operations
synapseclaw pairing init        # Nag-generate ng bagong pairing secret
synapseclaw pairing rotate      # Nag-rotate ng existing pairing secret

# Tunneling (para sa public exposure)
synapseclaw tunnel start        # Nagse-start ng tunnel sa local daemon
synapseclaw tunnel stop         # Naghihinto sa active tunnel

# Diagnostics
synapseclaw doctor              # Nagpapatakbo ng system health checks
synapseclaw version             # Nagpapakita ng version at build info
```
...
## Suportahan ang SynapseClaw

Kung tinutulungan ng SynapseClaw ang iyong trabaho at nais mong suportahan ang patuloy na development, maaari kang mag-donate dito:
...
**Ito ang tanging opisyal na SynapseClaw repository:**

> <https://github.com/panviktor/synapseclaw>

Ang anumang iba pang repository, organization, domain, o package na nagpapanggap na "SynapseClaw" o nagpapahiwatig ng affiliation sa SynapseClaw Labs ay **hindi awtorisado at hindi kaugnay sa proyektong ito**. Ang mga kilalang unauthorized forks ay ililista sa [TRADEMARK.md](TRADEMARK.md).

Kung makakita ka ng impersonation o trademark misuse, mangyaring [magbukas ng isyu](https://github.com/panviktor/synapseclaw/issues).

---

## License

Ang SynapseClaw ay dual-licensed para sa maximum openness at contributor protection:
...
Ang pangalang **SynapseClaw** at logo ay mga rehistradong trademark ng SynapseClaw Labs. Ang license na ito ay hindi nagbibigay ng pahintulot na gamitin ang mga ito upang ipahiwatig ang endorsement o affiliation. Tingnan ang [TRADEMARK.md](TRADEMARK.md) para sa mga allowed at prohibited uses.
...
- Bagong `Skill` → `~/.synapseclaw/workspace/skills/<n>/`
...
<p align="center">
  <a href="https://www.star-history.com/#panviktor/synapseclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Star History Graph" src="https://api.star-history.com/svg?repos=panviktor/synapseclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
