...
<p align="center">
  SynapseClaw es el <strong>sistema operativo de runtime</strong> para flujos de trabajo de agentes — una infraestructura que abstrae modelos, herramientas, memoria y ejecución para construir agentes una vez y ejecutarlos en cualquier lugar.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **No estamos afiliados** con `openagen/synapseclaw` o `synapseclaw.org`. El dominio `synapseclaw.org` apunta actualmente al fork `openagen/synapseclaw`, y este dominio/repositorio está suplantando nuestro sitio web/proyecto oficial.                                                                                                                                                                                 | No confíes en información, binarios, recaudaciones de fondos o anuncios de estas fuentes. Usa solo [este repositorio](https://github.com/panviktor/synapseclaw) y nuestras cuentas sociales verificadas.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Importante_ | Nuestro sitio web oficial ahora está en línea: [synapseclaw.dev](https://synapseclaw.dev). Gracias por tu paciencia durante la espera. Todavía detectamos intentos de suplantación: no participes en ninguna actividad de inversión/financiamiento en nombre de SynapseClaw si no se publica a través de nuestros canales oficiales.                                                                                                                   | Usa [este repositorio](https://github.com/panviktor/synapseclaw) como la única fuente de verdad. Sigue [X (@synapseclaw)](https://x.com/synapseclaw?s=21), [Telegram (@synapseclaw)](https://t.me/synapseclaw), [Facebook (grupo)](https://www.facebook.com/groups/synapseclaw), [Reddit (r/synapseclaw)](https://www.reddit.com/r/synapseclaw/), y [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) para actualizaciones oficiales. |
...
### Por qué los equipos eligen SynapseClaw
...
## Instantánea de Benchmark (SynapseClaw vs OpenClaw, Reproducible)
...
> Notas: Los resultados de SynapseClaw se miden en builds de producción usando `/usr/bin/time -l`. OpenClaw requiere el runtime Node.js (típicamente ~390 MB de sobrecarga de memoria adicional), mientras que NanoBot requiere el runtime Python. PicoClaw y SynapseClaw son binarios estáticos. Las cifras de RAM anteriores son memoria de runtime; los requisitos de compilación en tiempo de build son mayores.

<p align="center">
  <img src="zero-claw.jpeg" alt="Comparación SynapseClaw vs OpenClaw" width="800" />
</p>
...
- `synapseclaw --help`: tiempo real aprox `0.02s`, huella de memoria máxima ~`3.9 MB`
- `synapseclaw status`: tiempo real aprox `0.01s`, huella de memoria máxima ~`4.1 MB`
...
El script `bootstrap.sh` instala Rust, clona SynapseClaw, lo compila, y configura tu entorno de desarrollo inicial:
...
2. Clonará el repositorio SynapseClaw
3. Compilará SynapseClaw en modo release
4. Instalará `synapseclaw` en `~/.cargo/bin/`
5. Creará la estructura de workspace por defecto en `~/.synapseclaw/workspace/`
6. Generará un archivo de configuración inicial `~/.synapseclaw/workspace/config.toml`

Después del bootstrap, recarga tu shell o ejecuta `source ~/.cargo/env` para usar el comando `synapseclaw` globalmente.
...
1. Configura tus proveedores de AI en `~/.synapseclaw/workspace/config.toml`
2. Revisa la [referencia de configuración](docs/config-reference.md) para opciones avanzadas
3. Inicia el agente: `synapseclaw agent start`
4. Prueba vía tu canal preferido (ver [referencia de canales](docs/channels-reference.md))

## Configuración

Edita `~/.synapseclaw/workspace/config.toml` para configurar proveedores, canales y comportamiento del sistema.
...
SynapseClaw soporta dos backends de ejecución de código:
...
synapseclaw init                # Inicializa un nuevo workspace
synapseclaw status              # Muestra estado de daemon/agent
synapseclaw config validate     # Verifica sintaxis y valores de config.toml

# Gestión de daemon
synapseclaw daemon start        # Inicia el daemon en segundo plano
synapseclaw daemon stop         # Detiene el daemon en ejecución
synapseclaw daemon restart      # Reinicia el daemon (recarga de config)
synapseclaw daemon logs         # Muestra logs del daemon

# Gestión de agent
synapseclaw agent start         # Inicia el agent (requiere daemon ejecutándose)
synapseclaw agent stop          # Detiene el agent
synapseclaw agent restart       # Reinicia el agent (recarga de config)

# Operaciones de emparejamiento
synapseclaw pairing init        # Genera un nuevo secreto de emparejamiento
synapseclaw pairing rotate      # Rota el secreto de emparejamiento existente

# Tunneling (para exposición pública)
synapseclaw tunnel start        # Inicia un tunnel hacia el daemon local
synapseclaw tunnel stop         # Detiene el tunnel activo

# Diagnóstico
synapseclaw doctor              # Ejecuta verificaciones de salud del sistema
synapseclaw version             # Muestra versión e información de build
```
...
## Apoyar a SynapseClaw

Si SynapseClaw ayuda a tu trabajo y deseas apoyar el desarrollo continuo, puedes donar aquí:
...
**Este es el único repositorio oficial de SynapseClaw:**

> <https://github.com/panviktor/synapseclaw>

Cualquier otro repositorio, organización, dominio o paquete que afirme ser "SynapseClaw" o que implique afiliación con SynapseClaw Labs es **no autorizado y no está afiliado con este proyecto**. Los forks no autorizados conocidos serán listados en [TRADEMARK.md](TRADEMARK.md).

Si encuentras suplantación o uso indebido de marca, por favor [abre un issue](https://github.com/panviktor/synapseclaw/issues).

---

## Licencia

SynapseClaw tiene doble licencia para máxima apertura y protección de contribuyentes:
...
El nombre **SynapseClaw** y el logo son marcas registradas de SynapseClaw Labs. Esta licencia no otorga permiso para usarlos para implicar aprobación o afiliación. Ver [TRADEMARK.md](TRADEMARK.md) para usos permitidos y prohibidos.
...
- Nueva `Skill` → `~/.synapseclaw/workspace/skills/<n>/`

---

**SynapseClaw** — Cero sobrecarga. Cero compromiso. Despliega en cualquier lugar. Intercambia cualquier cosa. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#panviktor/synapseclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Gráfico de Historial de Estrellas" src="https://api.star-history.com/svg?repos=panviktor/synapseclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
