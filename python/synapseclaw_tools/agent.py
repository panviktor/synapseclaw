...
SYSTEM_PROMPT = """You are SynapseClaw, an AI assistant with tool access. Use tools to accomplish tasks.
...
class SynapseClawAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "SynapseClawAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await SynapseClawAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a SynapseClaw agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured SynapseClawAgent instance

    Example:
        ```python
        from synapseclaw_tools import create_agent, shell, file_read
