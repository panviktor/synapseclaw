...
Python companion package for [SynapseClaw](https://github.com/panviktor/synapseclaw) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install synapseclaw-tools[discord]
```
...
# Run the CLI
synapseclaw-tools "List files in the current directory"
...
```python
from synapseclaw_tools import tool
...
| Feature | Rust SynapseClaw | synapseclaw-tools |
|---------|---------------|----------------|
...
Use **Rust SynapseClaw** for production edge deployments. Use **synapseclaw-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
