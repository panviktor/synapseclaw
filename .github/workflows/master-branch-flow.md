...
SynapseClaw uses a single default branch: `master`. All contributor PRs target `master` directly. There is no `dev` or promotion branch.

Current maintainers with PR approval authority: `panviktor`.
...
4. Maintainer (`panviktor`) merges PR once checks and review policy are satisfied.
