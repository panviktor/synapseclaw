# SynapseClaw Development Environment

A fully containerized development sandbox for SynapseClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`synapseclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `synapseclaw` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/synapseclaw-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/synapseclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.synapseclaw` (mounted to `/synapseclaw-data/.synapseclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.synapseclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `synapseclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=synapseclaw-target`) to speed repeat local image builds.
