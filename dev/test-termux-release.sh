...
ASSET_URL="https://github.com/panviktor/synapseclaw/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t synapseclaw-termux-test-XXXXXX)"
...
# --- Test 5: Contains synapseclaw binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^synapseclaw$"; then
  pass "Archive contains 'synapseclaw' binary"
else
  fail "Archive does not contain 'synapseclaw' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/panviktor/synapseclaw/releases/download/${TAG}/SHA256SUMS"
