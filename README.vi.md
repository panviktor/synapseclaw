...
<p align="center">
  ...
  <a href="https://raw.githubusercontent.com/panviktor/synapseclaw/master/install.sh">Cài đặt một lần bấm</a> |
...
<p align="center">
  SynapseClaw là <strong>hệ điều hành runtime</strong> cho các quy trình làm việc của tác tử — cơ sở hạ tầng trừu tượng hóa mô hình, công cụ, bộ nhớ và thực thi để xây dựng tác tử một lần và chạy ở mọi nơi.
</p>
...
|---|---|---|---|
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không có liên kết** với `openagen/synapseclaw` hoặc `synapseclaw.org`. Tên miền `synapseclaw.org` hiện đang trỏ đến fork `openagen/synapseclaw`, và tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng [repository này](https://github.com/panviktor/synapseclaw) và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
| 2026-02-21 | _Quan trọng_ | Website chính thức của chúng tôi đã ra mắt: [synapseclaw.dev](https://synapseclaw.dev). Cảm ơn mọi người đã kiên nhẫn chờ đợi. Chúng tôi vẫn đang ghi nhận các nỗ lực mạo danh, vì vậy **không** tham gia bất kỳ hoạt động đầu tư hoặc gây quỹ nào nhân danh SynapseClaw nếu thông tin đó không được công bố qua các kênh chính thức của chúng tôi. | Sử dụng [repository này](https://github.com/panviktor/synapseclaw) làm nguồn thông tin duy nhất đáng tin cậy. Theo dõi [X (@synapseclaw)](https://x.com/synapseclaw?s=21), [Facebook (nhóm)](https://www.facebook.com/groups/synapseclaw), và [Reddit (r/synapseclaw)](https://www.reddit.com/r/synapseclaw/) để nhận cập nhật chính thức. |
...
### Vì sao các team chọn SynapseClaw
...
## So sánh hiệu suất (SynapseClaw vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả SynapseClaw được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), còn NanoBot yêu cầu runtime Python. PicoClaw và SynapseClaw là các static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build-time sẽ cao hơn.
...
- `synapseclaw --help`: khoảng `0.02s`, bộ nhớ đỉnh ~`3.9MB`
- `synapseclaw status`: khoảng `0.01s`, bộ nhớ đỉnh ~`4.1MB`
...
Hoặc bỏ qua các bước trên, cài hết mọi thứ (system deps, Rust, SynapseClaw) chỉ bằng một lệnh:
...
synapseclaw gateway                # mặc định: 127.0.0.1:42617
synapseclaw gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
SynapseClaw hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.synapseclaw/auth-profiles.json`
- Khóa mã hóa: `~/.synapseclaw/.secret_key`
...
Nếu SynapseClaw giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển liên tục, bạn có thể quyên góp tại đây:
...
**Đây là repository SynapseClaw chính thức duy nhất:**
> <https://github.com/panviktor/synapseclaw>

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "SynapseClaw" hoặc ngụ ý liên kết với SynapseClaw Labs đều là **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](docs/maintainers/trademark.md).

Nếu bạn phát hiện hành vi mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/panviktor/synapseclaw/issues).

---

## Giấy phép

SynapseClaw được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **SynapseClaw** và logo là nhãn hiệu của SynapseClaw Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](docs/maintainers/trademark.md) để biết các sử dụng được phép và bị cấm.
...
- `Skill` mới → `~/.synapseclaw/workspace/skills/<name>/`

---

**SynapseClaw** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
