...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/panviktor/synapseclaw/security/advisories/new)
...
SynapseClaw implements defense-in-depth security:
...
SynapseClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t synapseclaw .
docker inspect --format='{{.Config.User}}' synapseclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace synapseclaw gateway
```
