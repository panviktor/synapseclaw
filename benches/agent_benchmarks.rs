//! Performance benchmarks for SynapseClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/panviktor/synapseclaw/issues/618 (item 7)
...
use synapseclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use synapseclaw::config::MemoryConfig;
use synapseclaw::memory;
use synapseclaw::memory::{Memory, MemoryCategory};
use synapseclaw::observability::{NoopObserver, Observer};
use synapseclaw::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use synapseclaw::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "synapseclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "synapseclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about synapseclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("synapseclaw agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
