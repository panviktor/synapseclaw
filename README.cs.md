...
<p align="center">
  SynapseClaw je <strong>operační systém runtime</strong> pro workflow agentů — infrastruktura která abstrahuje modely, nástroje, paměť a provádění pro stavbu agentů jednou a spouštění kdekoliv.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritické_  | **Nejsme propojeni** s `openagen/synapseclaw` nebo `synapseclaw.org`. Doména `synapseclaw.org` aktuálně směřuje na fork `openagen/synapseclaw`, a tato doména/repoziťář se vydává za náš oficiální web/projekt.                                                                                                                                                                                 | Nevěřte informacím, binárním souborům, fundraisingu nebo oznámením z těchto zdrojů. Používejte pouze [tento repoziťář](https://github.com/panviktor/synapseclaw) a naše ověřené sociální účty.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Důležité_ | Náš oficiální web je nyní online: [synapseclaw.dev](https://synapseclaw.dev). Děkujeme za trpělivost během čekání. Stále detekujeme pokusy o vydávání se: neúčastněte žádné investiční/fundraisingové aktivity ve jménu SynapseClaw pokud není publikována přes naše oficiální kanály.                                                                                                                   | Používejte [tento repoziťář](https://github.com/panviktor/synapseclaw) jako jediný zdroj pravdy. Sledujte [X (@synapseclaw)](https://x.com/synapseclaw?s=21), [Telegram (@synapseclaw)](https://t.me/synapseclaw), [Facebook (skupina)](https://www.facebook.com/groups/synapseclaw), [Reddit (r/synapseclaw)](https://www.reddit.com/r/synapseclaw/), a [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) pro oficiální aktualizace. |
...
## Benchmark Snapshot (SynapseClaw vs OpenClaw, Reprodukovatelné)
...
> Poznámky: Výsledky SynapseClaw jsou měřeny na produkčních buildech pomocí `/usr/bin/time -l`. OpenClaw vyžaduje Node.js runtime (typicky ~390 MB dodatečného paměťového režijního nákladu), zatímco NanoBot vyžaduje Python runtime. PicoClaw a SynapseClaw jsou statická binaria. Výše uvedené RAM čísla jsou runtime paměť; build-time kompilační požadavky jsou vyšší.

<p align="center">
  <img src="zero-claw.jpeg" alt="Porovnání SynapseClaw vs OpenClaw" width="800" />
</p>
...
- `synapseclaw --help`: reálný čas přibližně `0.02s`, špičková paměťová stopa ~`3.9 MB`
- `synapseclaw status`: reálný čas přibližně `0.01s`, špičková paměťová stopa ~`4.1 MB`
...
Skript `bootstrap.sh` nainstaluje Rust, naklonuje SynapseClaw, zkompiluje ho a nastaví vaše počáteční vývojové prostředí:
...
2. Naklonuje SynapseClaw repoziťář
3. Zkompiluje SynapseClaw v release módu
4. Nainstaluje `synapseclaw` do `~/.cargo/bin/`
5. Vytvoří výchozí workspace strukturu v `~/.synapseclaw/workspace/`
6. Vygeneruje počáteční konfigurační soubor `~/.synapseclaw/workspace/config.toml`

Po bootstrapu znovu načtěte váš shell nebo spusťte `source ~/.cargo/env` pro použití příkazu `synapseclaw` globálně.
...
1. Nakonfigurujte své AI poskytovatele v `~/.synapseclaw/workspace/config.toml`
2. Podívejte se na [konfigurační referenci](docs/config-reference.md) pro pokročilé možnosti
3. Spusťte agenta: `synapseclaw agent start`
4. Otestujte přes váš preferovaný kanál (viz [kanálová reference](docs/channels-reference.md))

## Konfigurace

Upravte `~/.synapseclaw/workspace/config.toml` pro konfiguraci poskytovatelů, kanálů a chování systému.
...
SynapseClaw podporuje dva backendy provádění kódu:
...
```bash
# Správa workspace
synapseclaw init                # Inicializuje nový workspace
synapseclaw status              # Zobrazuje stav daemon/agent
synapseclaw config validate     # Ověřuje syntaxi a hodnoty config.toml

# Správa daemon
synapseclaw daemon start        # Spouští daemon na pozadí
synapseclaw daemon stop         # Zastavuje běžící daemon
synapseclaw daemon restart      # Restartuje daemon (znovunačtení config)
synapseclaw daemon logs         # Zobrazuje daemon logy

# Správa agent
synapseclaw agent start         # Spouští agenta (vyžaduje běžící daemon)
synapseclaw agent stop          # Zastavuje agenta
synapseclaw agent restart       # Restartuje agenta (znovunačtení config)

# Párovací operace
synapseclaw pairing init        # Generuje nové párovací tajemství
synapseclaw pairing rotate      # Rotuje existující párovací tajemství

# Tunneling (pro veřejnou expozici)
synapseclaw tunnel start        # Spouští tunnel k lokálnímu daemon
synapseclaw tunnel stop         # Zastavuje aktivní tunnel

# Diagnostika
synapseclaw doctor              # Spouští kontroly zdraví systému
synapseclaw version             # Zobrazuje verzi a build informace
```
...
## Podpořte SynapseClaw

Pokud SynapseClaw pomáhá vaší práci a chcete podpořit pokračující vývoj, můžete darovat zde:
...
**Toto je jediný oficiální SynapseClaw repoziťář:**

> <https://github.com/panviktor/synapseclaw>

Jakýkoliv jiný repoziťář, organizace, doména nebo balík tvrdící že je "SynapseClaw" nebo naznačující afiliaci s SynapseClaw Labs je **neautorizovaný a není spojen s tímto projektem**. Známé neautorizované forky budou uvedeny v [TRADEMARK.md](TRADEMARK.md).

Pokud narazíte na vydávání se nebo zneužití ochranné známky, prosím [otevřete issue](https://github.com/panviktor/synapseclaw/issues).

---

## Licence

SynapseClaw je duálně licencován pro maximální otevřenost a ochranu přispěvatelů:
...
Název **SynapseClaw** a logo jsou registrované ochranné známky SynapseClaw Labs. Tato licence neuděluje povolení je používat k naznačení schválení nebo afiliace. Viz [TRADEMARK.md](TRADEMARK.md) pro povolená a zakázaná použití.
...
- Nová `Skill` → `~/.synapseclaw/workspace/skills/<n>/`

---

**SynapseClaw** — Nulová režie. Nulové kompromisy. Nasazujte kdekoliv. Měňte cokoliv. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#panviktor/synapseclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Graf Historie Hvězd" src="https://api.star-history.com/svg?repos=panviktor/synapseclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
