...
use synapse_core::ports::approval::ApprovalPort;
...
pub(crate) fn filter_tool_specs_for_turn(
    ...
    groups: &[synapse_core::config::schema::ToolFilterGroup],
    user_message: &str,
) -> Vec<crate::tools::ToolSpec> {
    use synapse_core::config::schema::ToolFilterGroupMode;
    ...
    if let Ok(entries) = mem.recall(user_msg, 5, session_id).await {
        ...
        if !relevant.is_empty() {
            ...
            for entry in &relevant {
                ...
                if synapse_core::domain::util::should_skip_autosave_content(&entry.content) {
                    continue;
                }
...
async fn execute_one_tool(
    ...
    tool_middleware: Option<
        &std::sync::Arc<
            synapse_core::application::services::tool_middleware_service::ToolMiddlewareChain,
        >,
    >,
    ...
    if let Some(mw) = tool_middleware {
        let mw_ctx = synapse_core::domain::tool_middleware::ToolCallContext {
            run_id: None,
            pipeline_name: None,
            step_id: None,
            agent_id: String::new(),
            ...
            if let Some(port) = approval {
                if port.needs_approval(&tool_name) {
                    ...
                    let decision = match port.request_approval(&tool_name, &args_str).await {
                        Ok(resp) => resp,
                        Err(_) => synapse_core::domain::approval::ApprovalResponse::No,
                    };

                    let audit = synapse_core::domain::approval::ApprovalDecision {
                    ...
                    if decision == synapse_core::domain::approval::ApprovalResponse::No {
...
pub async fn run(
    ...
    if let Some(msg) = message {
        ...
        if config.memory.auto_save
            ...
            && !synapse_core::domain::util::should_skip_autosave_content(&msg)
        {
        ...
        loop {
            ...
            if config.memory.auto_save
                ...
                && !synapse_core::domain::util::should_skip_autosave_content(&user_input)
            {
                let user_key = autosave_memory_key("user_msg");
                let _ = mem
                    .store(
...
mod tests {
    ...
    async fn run_tool_call_loop_returns_structured_error_for_non_vision_provider() {
        ...
        let err = run_tool_call_loop(
            ...
            None,
            "cli",
            &synapse_core::config::schema::MultimodalConfig::default(),
            3,
            None,
            None,
            None,
    ...
    async fn run_tool_call_loop_rejects_oversized_image_payload() {
        ...
        let tools_registry: Vec<Box<dyn Tool>> = Vec::new();
        let observer = NoopObserver;
        let multimodal = synapse_core::config::schema::MultimodalConfig {
            max_images: 4,
            max_image_size_mb: 1,
    ...
    fn should_execute_tools_in_parallel_returns_false_when_approval_is_required() {
        ...
        let approval_cfg = synapse_core::config::schema::AutonomyConfig::default();
    ...
    fn should_execute_tools_in_parallel_returns_true_when_cli_has_no_interactive_approvals() {
        ...
        let approval_cfg = synapse_core::config::schema::AutonomyConfig {
...
{"name":"shell","arguments":{"command":"echo hello"}}
        ...
        let runtime: Arc<dyn synapse_core::ports::runtime::RuntimeAdapter> =
...
Let me check the result."#;
    ...
    fn filter_tool_specs_always_group_includes_matching_mcp_tool() {
        use synapse_core::config::schema::{ToolFilterGroup, ToolFilterGroupMode};
