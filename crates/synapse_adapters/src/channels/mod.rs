...
pub async fn handle_command(
    command: synapse_core::commands::ChannelCommands,
    config: &Config,
) -> Result<()> {
    match command {
        synapse_core::commands::ChannelCommands::Start => {
            anyhow::bail!("Start must be handled in main.rs (requires async runtime)")
        }
        synapse_core::commands::ChannelCommands::Doctor => {
            anyhow::bail!("Doctor must be handled in main.rs (requires async runtime)")
        }
        synapse_core::commands::ChannelCommands::List => {
            println!("Channels:");
        ...
        synapse_core::commands::ChannelCommands::Add {
            channel_type,
            config: _,
        } => {
            anyhow::bail!(
        ...
        synapse_core::commands::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.synapseclaw/config.toml directly");
        }
        synapse_core::commands::ChannelCommands::BindTelegram { identity } => {
            Box::pin(bind_telegram_identity(config, &identity)).await
        }
        synapse_core::commands::ChannelCommands::Send {
            message,
            channel_id,
            recipient,
        } => {
            ...
            if let Some(ref broker_token) = config.agents_ipc.broker_token {
                ...
                Some(
                    Arc::new(crate::pipeline::ipc_step_executor::IpcStepExecutor::new(
                        ...
                        as Arc<dyn synapse_core::ports::pipeline_executor::PipelineExecutorPort>,
                )
            } else {
                None
            }
