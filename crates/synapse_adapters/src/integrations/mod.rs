...
pub fn handle_command(
    command: synapse_core::commands::IntegrationCommands,
    config: &Config,
) -> Result<()> {
    match command {
        synapse_core::commands::IntegrationCommands::Info { name } => {
...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `synapseclaw onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: synapseclaw onboard --channels-only");
            println!("    4. Start: synapseclaw channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: synapseclaw onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: synapseclaw onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    SynapseClaw can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.synapseclaw/workspace/cron/");
            println!("    Run: synapseclaw cron list");
        }
        "Webhooks" => {
            println!("  Built-in:");
            println!("    HTTP endpoint for external triggers.");
            println!("    Run: synapseclaw gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/panviktor/synapseclaw");
            }
        }
    }
...
mod tests {
    ...
    fn handle_command_info_is_case_insensitive_for_known_integrations() {
        ...
        let result = handle_command(
            synapse_core::commands::IntegrationCommands::Info { name: first_name },
            &config,
        );
    ...
    fn handle_command_info_returns_error_for_unknown_integration() {
        let config = Config::default();
        let result = handle_command(
            synapse_core::commands::IntegrationCommands::Info {
