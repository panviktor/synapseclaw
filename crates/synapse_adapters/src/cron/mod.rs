...
pub fn handle_command(
    command: synapse_core::commands::CronCommands,
    config: &Config,
) -> Result<()> {
    match command {
        synapse_core::commands::CronCommands::List => {
        ...
        synapse_core::commands::CronCommands::Add {
            expression,
            tz,
            agent,
            command,
        ...
        synapse_core::commands::CronCommands::AddAt { at, agent, command } => {
        ...
        synapse_core::commands::CronCommands::AddEvery {
            every_ms,
            agent,
            command,
        } => {
        ...
        synapse_core::commands::CronCommands::Once {
            delay,
            agent,
            command,
        } => {
        ...
        synapse_core::commands::CronCommands::Update {
            id,
            expression,
            tz,
            command,
        ...
        synapse_core::commands::CronCommands::Remove { id } => remove_job(config, &id),
        synapse_core::commands::CronCommands::Pause { id } => {
            pause_job(config, &id)?;
            println!("⏸️  Paused cron job {id}");
            Ok(())
        }
        synapse_core::commands::CronCommands::Resume { id } => {
...
mod tests {
    ...
    fn add_once_validated_blocks_disallowed_command() {
        ...
        config.autonomy.level = synapse_core::domain::config::AutonomyLevel::Supervised;
