...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    if let Ok(custom_workspace) = std::env::var("SYNAPSECLAW_WORKSPACE") {
        ...
        if !trimmed.is_empty() {
            ...
            return synapse_core::config::workspace_io::resolve_config_dir_for_workspace(
...
async fn persist_workspace_selection(config_path: &Path) -> Result<()> {
    ...
    synapse_core::config::workspace_io::persist_active_workspace_config_dir(config_dir)
        .await
        .with_context(|| {
            format!(
...
async fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let (default_config_dir, default_workspace_dir) =
        synapse_core::config::workspace_io::resolve_runtime_dirs_for_onboarding().await?;
    ...
    let (config_dir, workspace_dir) = if use_default {
        ...
        synapse_core::config::workspace_io::resolve_config_dir_for_workspace(&PathBuf::from(
            expanded,
        ))
    };
