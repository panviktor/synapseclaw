...
Complete guide to setting up a SynapseClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.synapseclaw/models
cp models/ggml-base.bin ~/.synapseclaw/models/
```
...
mkdir -p ~/.synapseclaw/models/piper
cd ~/.synapseclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.synapseclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build SynapseClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/panviktor/synapseclaw
cd synapseclaw

# Build robot kit
cargo build --release -p synapseclaw-robot-kit

# Build main synapseclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.synapseclaw
nano ~/.synapseclaw/robot.toml
```

```toml
# ~/.synapseclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.synapseclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/synapseclaw_sensors.fifo"
...
### Start SynapseClaw Agent

```bash
# Configure SynapseClaw to use robot tools
cat > ~/.synapseclaw/config.toml << 'EOF'
...
cp ~/synapseclaw/crates/robot-kit/SOUL.md ~/.synapseclaw/workspace/

# Start agent
./target/release/synapseclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/synapseclaw_sensors.fifo ]; then
    mkfifo /tmp/synapseclaw_sensors.fifo
fi
...
# Start synapseclaw
cd ~/synapseclaw
./target/release/synapseclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/synapseclaw-robot.service
sudo tee /etc/systemd/system/synapseclaw-robot.service << 'EOF'
[Unit]
Description=SynapseClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/synapseclaw
...
sudo systemctl enable synapseclaw-robot
sudo systemctl start synapseclaw-robot

# Check status
sudo systemctl status synapseclaw-robot
journalctl -u synapseclaw-robot -f  # View logs
```
