# SynapseClaw Robot Kit

A complete toolkit for building AI-powered robots with SynapseClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build SynapseClaw with robot tools

```bash
# Clone and build
git clone https://github.com/panviktor/synapseclaw
cd synapseclaw
cargo build -p synapseclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.synapseclaw
cp crates/robot-kit/robot.toml ~/.synapseclaw/
cp crates/robot-kit/SOUL.md ~/.synapseclaw/workspace/
...
# Test in mock mode
./target/release/synapseclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/synapseclaw agent -m "Move forward 1 meter"
```
...
```rust
use synapseclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `synapseclaw`, add a thin adapter that maps this
...
mkdir synapseclaw-robot-kit
cp -r target/release/synapseclaw synapseclaw-robot-kit/
cp -r examples/robot_kit synapseclaw-robot-kit/
cp -r ~/.synapseclaw synapseclaw-robot-kit/dot-synapseclaw

# Include models
mkdir -p synapseclaw-robot-kit/models
cp ~/.synapseclaw/models/ggml-base.bin synapseclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf synapseclaw-robot-kit.tar.gz synapseclaw-robot-kit/

# Copy to USB
cp synapseclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
