...
use crate::domain::ipc::{self, AclError};
...
pub async fn send_message(
    ...
    let session_has_request = if kind == "result" {
        if let Some(sid) = session_id {
            bus.session_has_request(sid, from_agent)
                .await
                .unwrap_or(false)
        } else {
            false
...
pub fn check_session_limit(message_count: usize, max_exchanges: usize) -> bool {
...
/// Build the escalation payload for a session that exceeded its exchange limit.
///
/// Business rule: when a lateral session exceeds `max_exchanges`, an
/// escalation message is sent to the coordinator agent.
...
/// Send an escalation message to the coordinator when session limit is exceeded.
pub async fn escalate_session_limit(
    bus: &dyn IpcBusPort,
    from_agent: &str,
    to_agent: &str,
    session_id: &str,
    ...
    let payload = build_escalation_payload(
        session_id,
        from_agent,
        to_agent,
        exchange_count,
    ...
    bus.send_message(
        ...
        Some(session_id),
        from_trust_level,
        0, // default priority for escalations
    )
    .await
    .map_err(|e| AclError {
...
// ── Spawn result completion ─────────────────────────────────────

/// Check if an IPC result message should trigger spawn run completion.
///
/// Business rule: when an ephemeral child sends `kind=result` with a
/// session_id that matches a running spawn, the spawn should be completed.
pub fn should_complete_spawn(kind: &str, session_id: Option<&str>) -> bool {
    kind == "result" && session_id.is_some()
}
...
pub fn resolve_recipient(
    ...
    if from_trust_level >= 4 {
        l4_destinations.get(to).cloned().ok_or_else(|| AclError {
...
mod tests {
    ...
    async fn send_text_succeeds() {
        ...
        let seq = send_message(&bus, "a", "b", "text", "hello", None, 2, 0, &[], &[])
            .await
            .unwrap();
        assert_eq!(seq, 1);
    }
    ...
    async fn send_to_unknown_agent_fails() {
        ...
        let err = send_message(&bus, "a", "unknown", "text", "hello", None, 2, 0, &[], &[])
            .await
            .unwrap_err();
