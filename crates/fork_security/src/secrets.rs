...
// in `~/.synapseclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.synapseclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(synapseclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: synapseclaw_dir.join(".secret_key"),
            enabled,
        }
    }
