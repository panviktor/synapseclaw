...
/// Maps a single Nevis role to SynapseClaw permissions.
...
/// IAM policy engine that maps Nevis roles to SynapseClaw tool permissions.
///
    ...
    pub fn evaluate_tool_access(
        ...
        for role in &identity.roles {
            ...
            if let Some(compiled) = self.role_map.get(&key) {
                ...
                {
                    tracing::info!(
                        user_id = %crate::redact(&identity.user_id),
                        role = %key,
...
mod tests {
    ...
    fn test_mappings() -> Vec<RoleMapping> {
        vec![
            RoleMapping {
                ...
                synapseclaw_permissions: vec!["all".into()],
                workspace_access: vec!["all".into()],
            },
            RoleMapping {
                nevis_role: "operator".into(),
                synapseclaw_permissions: vec![
                    "shell".into(),
                    "file_read".into(),
                    "file_write".into(),
                    "memory_search".into(),
            ...
            RoleMapping {
                ...
                synapseclaw_permissions: vec!["file_read".into(), "memory_search".into()],
    ...
    fn duplicate_normalized_roles_are_rejected() {
        let mappings = vec![
            ...
            RoleMapping {
                nevis_role: " ADMIN ".into(),
                synapseclaw_permissions: vec!["file_read".into()],
