...
impl TranscriptionProvider for GroqProvider {
    ...
    async fn transcribe(&self, audio_data: &[u8], file_name: &str) -> Result<String> {
        ...
        let client = synapse_providers::proxy::build_runtime_proxy_client("transcription.groq");
...
impl OpenAiWhisperProvider {
    pub fn from_config(config: &synapse_domain::config::schema::OpenAiSttConfig) -> Result<Self> {
        let api_key = config
            .api_key
            .as_deref()
            .map(str::trim)
...
impl TranscriptionProvider for OpenAiWhisperProvider {
    ...
    async fn transcribe(&self, audio_data: &[u8], file_name: &str) -> Result<String> {
        ...
        let client = synapse_providers::proxy::build_runtime_proxy_client("transcription.openai");
...
impl DeepgramProvider {
    pub fn from_config(config: &synapse_domain::config::schema::DeepgramSttConfig) -> Result<Self> {
        let api_key = config
            .api_key
            .as_deref()
            .map(str::trim)
...
impl AssemblyAiProvider {
    pub fn from_config(
        config: &synapse_domain::config::schema::AssemblyAiSttConfig,
    ) -> Result<Self> {
        let api_key = config
            .api_key
            .as_deref()
...
impl GoogleSttProvider {
    pub fn from_config(config: &synapse_domain::config::schema::GoogleSttConfig) -> Result<Self> {
        let api_key = config
            .api_key
            .as_deref()
            .map(str::trim)
...
impl TranscriptionProvider for GoogleSttProvider {
    ...
    async fn transcribe(&self, audio_data: &[u8], file_name: &str) -> Result<String> {
        ...
        let client = synapse_providers::proxy::build_runtime_proxy_client("transcription.google");
...
mod tests {
    ...
    fn manager_registers_multiple_providers() {
        ...
        let mut config = TranscriptionConfig::default();
        config.api_key = Some("test-groq-key".to_string());
        config.openai = Some(synapse_domain::config::schema::OpenAiSttConfig {
