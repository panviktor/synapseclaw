...
impl TelegramChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        synapse_providers::proxy::build_runtime_proxy_client("channel.telegram")
    }
...
mod tests {
    ...
    fn with_transcription_sets_config_when_enabled() {
        let mut tc = synapse_domain::config::schema::TranscriptionConfig::default();
        tc.enabled = true;
    ...
    fn with_transcription_skips_when_disabled() {
        let tc = synapse_domain::config::schema::TranscriptionConfig::default(); // enabled = false
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false).with_transcription(tc);
    ...
    async fn e2e_live_voice_transcription_and_reply_cache() {
        ...
        let transcript: String =
            crate::transcription::transcribe_audio(audio_data, "hello.mp3", &config)
                .await
    ...
    fn photo_image_marker_detected_by_multimodal() {
        ...
        let messages = vec![synapse_providers::ChatMessage::user(
            photo_content.to_string(),
        )];
        let count = synapse_providers::multimodal::count_image_markers(&messages);
        assert_eq!(
            count, 1,
    ...
    fn e2e_attachment_saves_file_and_formats_content() {
        ...
        let doc_msgs = vec![synapse_providers::ChatMessage::user(doc_content)];
        assert_eq!(
            synapse_providers::multimodal::count_image_markers(&doc_msgs),
            0,
        ...
        let photo_msgs = vec![synapse_providers::ChatMessage::user(photo_content.clone())];
        assert_eq!(
            synapse_providers::multimodal::count_image_markers(&photo_msgs),
            1,
        ...
        let cap_msgs = vec![synapse_providers::ChatMessage::user(captioned.clone())];
        assert_eq!(
            synapse_providers::multimodal::count_image_markers(&cap_msgs),
            1,
        ...
        let md_msgs = vec![synapse_providers::ChatMessage::user(md_content)];
        assert_eq!(
            synapse_providers::multimodal::count_image_markers(&md_msgs),
            0,
            "markdown file must not trigger image marker detection"
        );
    }
    ...
    fn groq_provider_rejects_photo_with_vision_error() {
        ...
        let marker_count = synapse_providers::multimodal::count_image_markers(&messages);
