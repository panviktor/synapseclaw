...
impl OpenAiProvider {
    ...
    fn request_temperature_for_model(model: &str, requested_temperature: f64) -> f64 {
        model_catalog::model_request_policy("openai", model)
            .and_then(|policy| policy.fixed_temperature)
            .unwrap_or(requested_temperature)
    }
    ...
    fn http_client(&self) -> Client {
        crate::proxy::build_runtime_proxy_client_with_timeouts("provider.openai", 120, 10)
    }
}
...
impl Provider for OpenAiProvider {
    async fn chat_with_system(
        ...
        let adjusted_temperature = Self::request_temperature_for_model(model, temperature);
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = OpenAiProvider::new(None);
        let result = p
            .chat_with_system(Some("You are SynapseClaw"), "test", "gpt-4o", 0.5)
            .await;
        assert!(result.is_err());
    }
    ...
    fn request_serializes_with_system_message() {
        let req = ChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: "You are SynapseClaw".to_string(),
                },
                Message {
                    role: "user".to_string(),
    ...
    #[test]
    fn request_temperature_uses_catalog_policy_only() {
        assert_eq!(
            OpenAiProvider::request_temperature_for_model("o3", 0.7),
            1.0
        );
        assert_eq!(
            OpenAiProvider::request_temperature_for_model("openai/gpt-5", 0.2),
            1.0
        );
        assert_eq!(
            OpenAiProvider::request_temperature_for_model("catalog-unknown-model", 0.3),
            0.3
        );
    }
}
