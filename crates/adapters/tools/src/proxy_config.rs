    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                anyhow::anyhow!("Invalid scope '{scope}'. Use environment|synapseclaw|services")
            })?;
        }
        ...
        if proxy.enabled && proxy.scope == ProxyScope::Environment {
            ...
            synapse_providers::proxy::clear_proxy_process_env();
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage SynapseClaw proxy settings (scope: environment | synapseclaw | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | synapseclaw | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
