//! Security policy — domain types and pure validation logic.
//!
//! SecurityPolicy, ActionTracker, and all shell command / path validation
//! live here as pure domain logic with no infrastructure dependencies.

use parking_lot::Mutex;
use std::path::{Path, PathBuf};
use std::time::Instant;

use crate::domain::config::{AutonomyLevel, CommandRiskLevel, ToolOperation};
...
mod tests {
    ...
    // from_config tests live in src/security/policy.rs (depends on Config types)
    ...
    // from_config_creates_fresh_tracker test lives in src/security/policy.rs
