...
use synapse_domain::config::schema::OtpConfig;

const OTP_SECRET_FILE: &str = "otp-secret";
const OTP_DIGITS: u32 = 6;
const OTP_ISSUER: &str = "SynapseClaw";
...
impl OtpValidator {
    pub fn from_config(
        config: &OtpConfig,
        synapseclaw_dir: &Path,
        store: &SecretStore,
    ) -> Result<(Self, Option<String>)> {
        let secret_path = secret_file_path(synapseclaw_dir);
...
pub fn secret_file_path(synapseclaw_dir: &Path) -> PathBuf {
    synapseclaw_dir.join(OTP_SECRET_FILE)
}
