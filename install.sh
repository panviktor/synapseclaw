...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/synapseclaw-pacman.XXXXXX.conf)"
...
usage() {
  cat <<'USAGE'
SynapseClaw installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds SynapseClaw, configures your provider and API key,
...
Examples:
  ...
  curl -fsSL https://synapseclaw.dev/install.sh | bash
...
Environment:
  SYNAPSECLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  SYNAPSECLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  SYNAPSECLAW_DOCKER_IMAGE      Docker image tag to build/run (default: synapseclaw-bootstrap:local)
  SYNAPSECLAW_API_KEY           Used when --api-key is not provided
  SYNAPSECLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  SYNAPSECLAW_MODEL             Used when --model is not provided
  SYNAPSECLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  SYNAPSECLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  SYNAPSECLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${SYNAPSECLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${SYNAPSECLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/panviktor/synapseclaw/releases"
...
install_prebuilt_binary() {
  ...
  asset_name="synapseclaw-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/panviktor/synapseclaw/releases/latest/download/${asset_name}"
  fi

  temp_dir="$(mktemp -d -t synapseclaw-prebuilt-XXXXXX)"
  ...
  extracted_bin="$temp_dir/synapseclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name synapseclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable synapseclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/synapseclaw"
  rm -rf "$temp_dir"

  step_ok "Installed pre-built binary to $install_dir/synapseclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/synapseclaw-pacman.XXXXXX.conf)"
...
prompt_api_key() {
  ...
  if [[ -n "$api_key_input" ]]; then
    ...
    warn "No API key entered — you can configure it later with synapseclaw onboard"
    SKIP_ONBOARD=true
  fi
}
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} SynapseClaw Guided Installer${RESET}"
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${SYNAPSECLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set SYNAPSECLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set SYNAPSECLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${SYNAPSECLAW_DOCKER_IMAGE:-synapseclaw-bootstrap:local}"
  fallback_image="ghcr.io/panviktor/synapseclaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.synapseclaw-docker"
  else
    default_data_dir="$WORK_DIR/.synapseclaw-docker"
  fi
  docker_data_dir="${SYNAPSECLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.synapseclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official SynapseClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.synapseclaw:/synapseclaw-data/.synapseclaw"
  workspace_mount="$docker_data_dir/workspace:/synapseclaw-data/workspace"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run synapseclaw onboard inside the container to configure."
  fi
}
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/panviktor/synapseclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${SYNAPSECLAW_CONTAINER_CLI:-docker}"
API_KEY="${SYNAPSECLAW_API_KEY:-}"
PROVIDER="${SYNAPSECLAW_PROVIDER:-openrouter}"
MODEL="${SYNAPSECLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${SYNAPSECLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set SYNAPSECLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t synapseclaw-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} SynapseClaw Installer${RESET}"
...
EXISTING_VERSION=""
INSTALL_MODE="fresh"
if have_cmd synapseclaw; then
  EXISTING_VERSION="$(synapseclaw --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
elif [[ -x "$HOME/.cargo/bin/synapseclaw" ]]; then
  EXISTING_VERSION="$("$HOME/.cargo/bin/synapseclaw" --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
fi
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing SynapseClaw installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing SynapseClaw installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized SynapseClaw data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}synapseclaw status${RESET}"
  echo -e "  ${DIM}synapseclaw agent -m \"Hello, SynapseClaw!\"${RESET}"
  echo -e "  ${DIM}synapseclaw gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.synapseclaw.dev/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing SynapseClaw${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing SynapseClaw v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  step_dot "Installing synapseclaw to cargo bin"

  # Clean up stale cargo install tracking from the old "synapseclaw" package name
  # (renamed to "synapseclaw"). Without this, `cargo install synapseclaw` from
  # crates.io fails with "binary already exists as part of `synapseclaw`".
  if have_cmd cargo; then
    if [[ -f "$HOME/.cargo/.crates.toml" ]] && grep -q '^"synapseclaw ' "$HOME/.cargo/.crates.toml" 2>/dev/null; then
      step_dot "Removing stale cargo tracking for old 'synapseclaw' package name"
      cargo uninstall synapseclaw 2>/dev/null || true
    fi
  fi

  cargo install --path "$WORK_DIR" --force --locked
  step_ok "SynapseClaw installed"

  # Sync binary to ~/.local/bin so PATH lookups find the fresh version
  if [[ -d "$HOME/.local/bin" ]]; then
    cp -f "$HOME/.cargo/bin/synapseclaw" "$HOME/.local/bin/synapseclaw" 2>/dev/null && \
...
SYNAPSECLAW_BIN=""
if [[ -x "$HOME/.cargo/bin/synapseclaw" ]]; then
  SYNAPSECLAW_BIN="$HOME/.cargo/bin/synapseclaw"
elif [[ -x "$WORK_DIR/target/release/synapseclaw" ]]; then
  SYNAPSECLAW_BIN="$WORK_DIR/target/release/synapseclaw"
elif have_cmd synapseclaw; then
  SYNAPSECLAW_BIN="synapseclaw"
fi
...
if [[ "$SKIP_ONBOARD" == false && -n "$SYNAPSECLAW_BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    ...
    if "$SYNAPSECLAW_BIN" onboard --provider ollama 2>/dev/null; then
  ...
  step_dot "Skipping configuration (run synapseclaw onboard later)"
elif [[ -z "$SYNAPSECLAW_BIN" ]]; then
  warn "SynapseClaw binary not found — cannot configure provider"
fi

# --- Gateway service management ---
if [[ -n "$SYNAPSECLAW_BIN" ]]; then
  # Try to install and start the gateway service
  step_dot "Checking gateway service"
  if "$SYNAPSECLAW_BIN" service install 2>/dev/null; then
    step_ok "Gateway service installed"
    if "$SYNAPSECLAW_BIN" service restart 2>/dev/null; then
      ...
      for i in 1 2 3 4 5; do
        sleep 2
        if PAIR_CODE=$("$SYNAPSECLAW_BIN" gateway get-paircode 2>/dev/null | grep -oE '[0-9]{6}'); then
          break
        fi
      done
      if [[ -n "$PAIR_CODE" ]]; then
        ...
        echo -e "  ${DIM}Run 'synapseclaw gateway get-paircode --new' anytime to generate a fresh code.${RESET}"
      fi
    else
      step_fail "Gateway service restart failed — re-run with synapseclaw service start"
    fi
  else
    step_dot "Gateway service not installed (run synapseclaw service install later)"
  fi

  # --- Post-install doctor check ---
  step_dot "Running doctor to validate installation"
  if "$SYNAPSECLAW_BIN" doctor 2>/dev/null; then
    step_ok "Doctor complete"
  else
    warn "Doctor reported issues — run synapseclaw doctor --fix to resolve"
  fi
fi
...
INSTALLED_VERSION=""
if [[ -n "$SYNAPSECLAW_BIN" ]]; then
  INSTALLED_VERSION="$("$SYNAPSECLAW_BIN" --version 2>/dev/null | awk '{print $NF}' || true)"
fi

# --- Success banner ---
echo
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} SynapseClaw installed successfully (SynapseClaw ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} SynapseClaw installed successfully!${RESET}"
fi

if [[ -x "$HOME/.cargo/bin/synapseclaw" ]] && ! have_cmd synapseclaw; then
  echo
  warn "synapseclaw is installed in $HOME/.cargo/bin, but that directory is not in PATH for this shell."
...
echo -e "${DIM}  Run 'synapseclaw gateway get-paircode' to get your pairing code.${RESET}"
