...
File a structured GitHub issue (bug report or feature request) for SynapseClaw interactively from Claude Code.

## When to Use

Trigger when the user wants to file a GitHub issue, report a bug, or request a feature for SynapseClaw. Keywords: "file issue", "report bug", "feature request", "open issue", "create issue", "github issue".

## Instructions

You are filing a GitHub issue against the SynapseClaw repository using structured issue forms. Follow this workflow exactly.
...
cargo metadata --format-version=1 --no-deps 2>/dev/null | jq -r '.packages[] | select(.name=="synapseclaw") | .version' 2>/dev/null   # SynapseClaw version
...
- **Use neutral project-scoped placeholders** per SynapseClaw's privacy contract.
