...
- variable: `HOMEBREW_CORE_BOT_FORK_REPO` (for example `synapseclaw-release-bot/homebrew-core`)
...
- variable: `SCOOP_BUCKET_REPO` (for example `panviktor/scoop-synapseclaw`)

Workflow guardrails:

- release tag must be `vX.Y.Z` format
- Windows binary SHA256 extracted from `SHA256SUMS` release asset
- manifest pushed to `bucket/synapseclaw.json` in the Scoop bucket repo
...
- PKGBUILD and .SRCINFO pushed to AUR `synapseclaw` package
