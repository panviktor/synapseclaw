# Nucleo-F401RE 上的 SynapseClaw — 分步指南

在 Mac 或 Linux 主机上运行 SynapseClaw。通过 USB 连接 Nucleo-F401RE。通过 Telegram 或 CLI 控制 GPIO（LED、引脚）。

---

## 通过 Telegram 获取开发板信息（无需固件）

SynapseClaw 可以通过 USB 从 Nucleo 读取芯片信息，**无需烧录任何固件**。向你的 Telegram 机器人发送消息：
...
SynapseClaw 包含 Nucleo-F401RE 所需的一切：
...
| 烧录命令 | `synapseclaw peripheral flash-nucleo` | 构建固件，通过 probe-rs 烧录 |
...
### 1.2 通过 SynapseClaw 烧录

在 synapseclaw 仓库根目录执行：
...
| 5 | `synapseclaw daemon` 或 `synapseclaw agent -m "Turn on LED"` |
...
- **GPIO 命令被忽略** — 检查配置中的 `path` 与你的串口匹配。运行 `synapseclaw peripheral list` 验证。
