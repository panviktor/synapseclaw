...
SynapseClaw 让微控制器（MCU，Microcontroller Unit）和单板计算机（SBC，Single Board Computer）能够**动态解释自然语言命令**，生成硬件特定代码，并实时执行外设交互。

## 1. 愿景

**目标：** SynapseClaw 作为具备硬件感知能力的 AI 代理，能够：
...
**思维模型：** SynapseClaw = 理解硬件的大脑。外设 = 它控制的手臂和腿。
...
SynapseClaw **直接运行在设备上**。开发板启动 gRPC/nanoRPC 服务器，与本地外设通信。
...
2. SynapseClaw 获取开发板特定文档（例如 ESP32 GPIO 映射）
3. LLM 合成 Rust 代码
...
SynapseClaw 运行在**主机**上，并维护到目标的硬件感知链接。用于开发、内省和烧录。
...
2. SynapseClaw 识别连接的硬件（VID/PID、架构）
3. 执行内存映射；建议可用的地址空间
4. 将结果返回给用户

**或：**
1. 用户：*"将这个固件烧录到 Nucleo"*
2. SynapseClaw 通过 OpenOCD 或 probe-rs 写入/烧录
3. 确认成功

**或：**
1. SynapseClaw 自动发现：*"STM32 Nucleo 位于 /dev/ttyACM0，ARM Cortex-M4"*
...
| 方面           | 边缘原生                    | 主机介导                    |
|------------------|--------------------------------|----------------------------------|
| SynapseClaw 运行位置 | 设备（ESP32、树莓派）           | 主机（Mac、Linux）                |
...
主机运行 SynapseClaw；外设运行最小化固件。通过串口传输简单 JSON。

### 模式 B：树莓派作为主机（原生 GPIO）

SynapseClaw 运行在树莓派上；通过 rppal 或 sysfs 访问 GPIO。不需要单独的固件。
...
1. **启动：** SynapseClaw 加载配置，读取 `peripherals.boards`。
...
用于 SynapseClaw 和外设之间的低延迟、类型化 RPC：
...
- **synapseclaw-firmware** 或 **synapseclaw-peripheral** —— 独立的 crate/工作区。
- 目标：`thumbv7em-none-eabihf`（STM32）、`armv7-unknown-linux-gnueabihf`（树莓派）等。
- STM32 使用 `embassy` 或 Zephyr。
- 实现上述协议。
- 用户将其烧录到开发板；SynapseClaw 连接并发现能力。

## 9. 实现阶段

### 阶段 1：骨架 ✅（已完成）

- [x] 添加 `Peripheral` 特征、配置 schema、CLI（`synapseclaw peripheral list/add`）
- [x] 为代理添加 `--peripheral` 标志
- [x] 在 AGENTS.md 中记录

### 阶段 2：主机介导 — 硬件发现 ✅（已完成）

- [x] `synapseclaw hardware discover`：枚举 USB 设备（VID/PID）
- [x] 开发板注册表：映射 VID/PID → 架构、名称（例如 Nucleo-F401RE）
- [x] `synapseclaw hardware introspect <path>`：内存映射、外设列表
...
- [x] 树莓派上的 SynapseClaw（通过 rppal 实现原生 GPIO）
...
- [ ] ESP32 上运行 SynapseClaw（Wi-Fi + LLM，边缘原生）—— 未来
...
- 在裸 STM32 上运行完整 SynapseClaw（无 Wi-Fi、RAM 有限）—— 改用主机介导模式
- 实时保证 —— 外设是尽力而为的
...
> *"像 ESP、树莓派或带 Wi-Fi 的开发板可以连接到 LLM（Gemini 或开源模型）。SynapseClaw 运行在设备上，创建自己的 gRPC 服务，启动服务并与外设通信。用户通过 WhatsApp 询问：'移动 X 机械臂'或'打开 LED'。SynapseClaw 获取准确的文档，编写代码，执行它，优化存储，运行并打开 LED —— 所有操作都在开发板上完成。*
>
> *对于通过 USB/J-Link/Aardvark 连接到我 Mac 的 STM Nucleo：我 Mac 上的 SynapseClaw 访问硬件，在设备上安装或写入想要的内容，并返回结果。示例：'嘿 SynapseClaw，这个 USB 设备上的可用/可读地址是什么？'它能找出连接的内容和位置并给出建议。"*
