...
本页面介绍安装和初始化 SynapseClaw 的最快支持路径。
...
默认行为是**仅应用程序**（编译/安装 SynapseClaw），需要已存在 Rust 工具链。
...
这会构建本地 SynapseClaw 镜像并在容器内启动引导流程，同时将配置/工作区持久化到 `./.synapseclaw-docker`。

容器 CLI 默认为 `docker`。如果 Docker CLI 不可用且存在 `podman`，安装程序会自动回退到 `podman`。你也可以显式设置 `SYNAPSECLAW_CONTAINER_CLI`（例如：`SYNAPSECLAW_CONTAINER_CLI=podman ./install.sh --docker`）。

对于 Podman，安装程序会使用 `--userns keep-id` 和 `:Z` 卷标签，确保工作区/配置挂载在容器内保持可写。

如果你添加 `--skip-build` 参数，安装程序会跳过本地镜像构建。它会首先尝试本地 Docker 标签（`SYNAPSECLAW_DOCKER_IMAGE`，默认：`synapseclaw-bootstrap:local`）；如果不存在，会拉取 `ghcr.io/panviktor/synapseclaw:latest` 并在运行前打本地标签。
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build`（在 `--docker` 模式下：如果存在使用本地镜像，否则拉取 `ghcr.io/panviktor/synapseclaw:latest`）
- `--skip-install`
- `--provider <id>`
