...
SynapseClaw 通过 REST API v4 原生支持与 Mattermost 集成。这种集成非常适合需要自主可控通信的自托管、私有或隔离网络环境。
...
| `mention_only` | （可选）当为 `true` 时，仅处理显式@机器人用户名的消息（例如 `@synapseclaw-bot`）。默认：`false`。 |

## 话题对话

SynapseClaw 在两种模式下都支持 Mattermost 话题：
- 如果用户在现有话题中发送消息，SynapseClaw 始终在同一个话题中回复。
- 如果 `thread_replies = true`（默认），顶层消息会通过创建话题来回复。
- 如果 `thread_replies = false`，顶层消息会在频道根层级回复。

## 仅@模式

当 `mention_only = true` 时，SynapseClaw 在 `allowed_users` 授权后会应用额外的过滤：
