...
本指南介绍 SynapseClaw 的原生 Nextcloud Talk 集成。
...
在 `~/.synapseclaw/config.toml` 中添加以下部分：
...
- 设置 `SYNAPSECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` 时会覆盖 `webhook_secret`。
...
配置 `webhook_secret` 时，SynapseClaw 会验证：
...
- SynapseClaw 忽略来自机器人的 webhook 事件（`actorType = bots`）。
- SynapseClaw 忽略非消息/系统事件。
