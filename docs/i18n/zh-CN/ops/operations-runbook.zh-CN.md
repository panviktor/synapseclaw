...
| 模式 | 命令 | 使用场景 |
|---|---|---|
| 前台运行时 | `synapseclaw daemon` | 本地调试、短期会话 |
| 仅前台网关 | `synapseclaw gateway` | webhook 端点测试 |
| 用户服务 | `synapseclaw service install && synapseclaw service start` | 持久化运维管理的运行时 |
...
| 信号 | 命令 / 文件 | 预期结果 |
|---|---|---|
| 配置有效性 | `synapseclaw doctor` | 无严重错误 |
| 渠道连通性 | `synapseclaw channel doctor` | 配置的渠道健康 |
| 运行时摘要 | `synapseclaw status` | 预期的提供商/模型/渠道 |
| 守护进程心跳/状态 | `~/.synapseclaw/daemon_state.json` | 文件定期更新 |
...
4. 如果渠道仍然失败，验证 `~/.synapseclaw/config.toml` 中的白名单和凭证。
...
1. 备份 `~/.synapseclaw/config.toml`
2. 每次只应用一个逻辑变更
3. 运行 `synapseclaw doctor`
4. 重启守护进程/服务
