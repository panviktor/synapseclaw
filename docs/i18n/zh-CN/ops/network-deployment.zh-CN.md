# 网络部署 — 树莓派和本地网络上的 SynapseClaw

本文档介绍如何在树莓派或本地网络上的其他主机上部署 SynapseClaw，支持 Telegram 和可选的 webhook 渠道。
...
| 模式 | 需要入站端口？ | 使用场景 |
|------|----------------------|----------|
| **Telegram 轮询** | 否 | SynapseClaw 轮询 Telegram API；可在任何地方工作 |
| **Matrix 同步（包括 E2EE）** | 否 | SynapseClaw 通过 Matrix 客户端 API 同步；不需要入站 webhook |
...
**关键点：** Telegram、Discord、Slack 和 Nostr 使用**出站连接** — SynapseClaw 连接到外部服务器/中继。不需要端口转发或公共 IP。
...
- Telegram 渠道工作正常：SynapseClaw 轮询 Telegram API（出站）
- 不需要防火墙或端口转发
...
2. 启动隧道：
   ...
   ```
   或使用 `synapseclaw tunnel`（参见隧道文档）。

3. 除非 `allow_public_bind = true` 或隧道处于活动状态，否则 SynapseClaw 会拒绝绑定到 `0.0.0.0`。
...
- SynapseClaw 调用 `https://api.telegram.org/bot{token}/getUpdates`
- 不需要入站端口或公共 IP
...
运行 `synapseclaw daemon` — Telegram 渠道会自动启动。
...
- 为同一个令牌仅保留一个运行时实例（推荐：`synapseclaw daemon` 服务）。
...
- [ ] 运行 `synapseclaw daemon --host 127.0.0.1 --port 42617`（Telegram 不需要 0.0.0.0 即可工作）
...
SynapseClaw 支持 Alpine Linux 和其他使用 OpenRC 初始化系统的发行版的 OpenRC。OpenRC 服务**系统级**运行，需要 root/sudo。

### 7.1 前置条件

- Alpine Linux（或其他基于 OpenRC 的发行版）
- Root 或 sudo 访问权限
- 专用的 `synapseclaw` 系统用户（安装期间创建）
...
这会创建：
- 初始化脚本：`/etc/init.d/synapseclaw`
- 配置目录：`/etc/synapseclaw/`
- 日志目录：`/var/log/synapseclaw/`

### 7.3 配置

通常不需要手动复制配置。

`sudo synapseclaw service install` 会自动准备 `/etc/synapseclaw`，如果有可用的用户设置，会迁移现有运行时状态，并为 `synapseclaw` 服务用户设置所有权/权限。

如果没有可迁移的现有运行时状态，请在启动服务前创建 `/etc/synapseclaw/config.toml`。
...
| 命令 | 描述 |
|---------|-------------|
| `sudo rc-service synapseclaw start` | 启动守护进程 |
| `sudo rc-service synapseclaw stop` | 停止守护进程 |
| `sudo rc-service synapseclaw status` | 检查服务状态 |
| `sudo rc-service synapseclaw restart` | 重启守护进程 |
| `sudo synapseclaw service status` | SynapseClaw 状态包装器（使用 `/etc/synapseclaw` 配置） |
...
| 日志 | 路径 |
|-----|------|
| 访问/stdout | `/var/log/synapseclaw/access.log` |
| 错误/stderr | `/var/log/synapseclaw/error.log` |
...
- 服务以 `synapseclaw:synapseclaw` 用户运行（最小权限原则）
- 配置必须位于 `/etc/synapseclaw/config.toml`（初始化脚本中的显式路径）
- 如果 `synapseclaw` 用户不存在，安装会失败并提供创建说明

### 7.9 检查清单：Alpine/OpenRC 部署

- [ ] 安装：`sudo synapseclaw service install`
- [ ] 启用：`sudo rc-update add synapseclaw default`
- [ ] 启动：`sudo rc-service synapseclaw start`
- [ ] 验证：`sudo rc-service synapseclaw status`
- [ ] 检查日志：`/var/log/synapseclaw/error.log`
