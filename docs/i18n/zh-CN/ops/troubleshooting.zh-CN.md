...
- `cargo check` / `cargo build` 似乎长时间卡在 `Checking synapseclaw`
- 重复出现 `Blocking waiting for file lock on package cache` 或 `build directory`

SynapseClaw 中出现此问题的原因：
...
### 安装后找不到 `synapseclaw` 命令

症状：

- 安装成功，但 shell 找不到 `synapseclaw`
...
- 为该令牌仅保留一个活动运行时
- 停止额外的 `synapseclaw daemon` / `synapseclaw channel start` 进程
