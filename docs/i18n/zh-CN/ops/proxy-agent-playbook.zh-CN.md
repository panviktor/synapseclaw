...
- **受众：** 在代理网络中运行 SynapseClaw 的运维人员和维护者。
- **范围：** `proxy_config` 操作、模式选择、验证流程和故障排除。
- **非目标：** SynapseClaw 运行时行为之外的通用网络调试。
...
- [第 4 节](#4-模式-a--仅代理-synapseclaw-内部流量)
...
| 范围 | 影响 | 导出环境变量 | 典型用途 |
|---|---|---|---|
| `synapseclaw` | SynapseClaw 内部 HTTP 客户端 | 否 | 无进程级副作用的正常运行时代理 |
...
## 4. 模式 A — 仅代理 SynapseClaw 内部流量

当 SynapseClaw 提供商/渠道/工具 HTTP 流量应使用代理，但不导出进程级代理环境变量时使用。
...
- SynapseClaw HTTP 客户端的运行时代理处于活动状态。
