...
SynapseClaw 是一个以 Rust 为优先开发语言的自主代理运行时。它从消息平台接收消息，经由 LLM 路由，执行工具调用，持久化内存，并返回响应。它还可以控制硬件外设并作为长期运行的守护进程。
...
| 文件 | 行数 | 角色 |
|---|---|---|
| `main.rs` | 1,977 | CLI 入口点。Clap 解析器，命令分发。所有 `synapseclaw <子命令>` 路由都在此处。 |
...
| 模块 | 关键文件 | 角色 |
|---|---|---|
| `skills/` | `mod.rs` (1.5k)、`audit.rs` | **用户/社区创作的能力。** 从 `~/.synapseclaw/workspace/skills/<name>/SKILL.md` 加载。CLI 命令：列表、安装、审计、移除。可选从开放技能仓库同步社区内容。 |
