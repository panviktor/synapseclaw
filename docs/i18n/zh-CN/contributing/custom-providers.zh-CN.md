...
SynapseClaw 支持兼容 OpenAI 和兼容 Anthropic 的自定义 API 端点。
...
# 或：export SYNAPSECLAW_API_KEY="your-api-key"
synapseclaw agent
```

## llama.cpp 服务器（推荐本地设置）

SynapseClaw 包含 `llama-server` 的一流本地提供商支持：
...
此流程不需要导出 `SYNAPSECLAW_API_KEY=dummy`。

## SGLang 服务器

SynapseClaw 包含 [SGLang](https://github.com/sgl-project/sglang) 的一流本地提供商支持：
...
SynapseClaw 包含 [vLLM](https://docs.vllm.ai/) 的一流本地提供商支持：
