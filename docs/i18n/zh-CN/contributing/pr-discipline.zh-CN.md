...
SynapseClaw 拉取请求的质量、署名、隐私和交接规则。
...
- 如果不可避免需要类似身份的上下文，仅使用 SynapseClaw 范围的角色/标签（例如 `SynapseClawAgent`、`SynapseClawOperator`、`synapseclaw_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`SynapseClawAgent`、`SynapseClawOperator`、`SynapseClawMaintainer`、`synapseclaw_user`
    - 服务/运行时标签：`synapseclaw_bot`、`synapseclaw_service`、`synapseclaw_runtime`、`synapseclaw_node`
    - 环境标签：`synapseclaw_project`、`synapseclaw_workspace`、`synapseclaw_channel`
