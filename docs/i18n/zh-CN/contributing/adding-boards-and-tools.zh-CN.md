# 添加开发板和工具 — SynapseClaw 硬件指南

本指南解释如何向 SynapseClaw 添加新的硬件开发板和自定义工具。

## 快速开始：通过 CLI 添加开发板

```bash
# 添加开发板（更新 ~/.synapseclaw/config.toml）
synapseclaw peripheral add nucleo-f401re /dev/ttyACM0
synapseclaw peripheral add arduino-uno /dev/cu.usbmodem12345
synapseclaw peripheral add rpi-gpio native   # 用于树莓派 GPIO（Linux）
...
使用 `rag-pdf` 特性时，SynapseClaw 可以索引 PDF 文件：
...
2. **添加到配置** — `synapseclaw peripheral add my-board /dev/ttyUSB0`
...
| 命令 | 描述 |
|---------|-------------|
| `synapseclaw peripheral list` | 列出已配置的开发板 |
| `synapseclaw peripheral add <board> <path>` | 添加开发板（写入配置） |
| `synapseclaw peripheral flash` | 烧录 Arduino 固件 |
| `synapseclaw peripheral flash-nucleo` | 烧录 Nucleo 固件 |
| `synapseclaw hardware discover` | 列出 USB 设备 |
| `synapseclaw hardware info` | 通过 probe-rs 获取芯片信息 |
