...
本指南介绍如何在 Matrix 房间（包括端到端加密 (E2EE) 房间）中可靠运行 SynapseClaw。
...
3. `room_id` 正确：
   ...
   - 支持：房间别名（`#alias:server`），SynapseClaw 会解析它
...
- SynapseClaw 尝试从 Matrix `/_matrix/client/v3/account/whoami` 读取身份信息。
...
3. 确认 SynapseClaw 日志包含 Matrix 监听器启动信息，没有重复的同步/认证错误。
...
- SynapseClaw 将 Matrix 文本回复作为支持 markdown 的 `m.room.message` 文本内容发送。
- 支持 `formatted_body` 的 Matrix 客户端应渲染强调、列表和代码块。
- 如果格式显示为纯文本，首先检查客户端能力，然后确认 SynapseClaw 运行的构建包含启用 markdown 的 Matrix 输出。
