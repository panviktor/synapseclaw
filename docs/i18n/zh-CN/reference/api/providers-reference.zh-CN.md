...
1. 配置/CLI 中的显式凭证
2. 提供商特定的环境变量
3. 通用回退环境变量：`SYNAPSECLAW_API_KEY` 然后是 `API_KEY`
...
- 多模态归一化后，SynapseClaw 通过 Ollama 原生的 `messages[].images` 字段发送图像负载。
- 如果选择了不支持视觉的提供商，SynapseClaw 会返回结构化能力错误，而不是静默忽略图像。

### Ollama 云路由说明

- 仅在使用远程 Ollama 端点时使用 `:cloud` 模型后缀。
- 远程端点应在 `api_url` 中设置（例如：`https://ollama.com`）。
- SynapseClaw 会自动归一化 `api_url` 中末尾的 `/api`。
...
- 模型发现：`synapseclaw models refresh --provider llamacpp`
...
- 模型发现：`synapseclaw models refresh --provider sglang`
...
- 模型发现：`synapseclaw models refresh --provider vllm`
...
- 模型发现：`synapseclaw models refresh --provider osaurus`
...
- 模型发现：`synapseclaw models refresh --provider nvidia`
