...
6. **运行时状态陈旧**：配置已更改但 `synapseclaw daemon` 未重启。

---

## 1. 配置命名空间

所有渠道设置都位于 `~/.synapseclaw/config.toml` 的 `channels_config` 下。
...
运行 `synapseclaw channel start`（或守护进程模式）时，Telegram 和 Discord 现在支持发送者范围的运行时切换：
...
- 模型缓存预览来自 `synapseclaw models refresh --provider <ID>`。
- 这些是运行时聊天命令，不是 CLI 子命令。

## 入站图像标记协议

SynapseClaw 通过内联消息标记支持多模态输入：
...
如果存在 `[channels_config.matrix]`、`[channels_config.lark]` 或 `[channels_config.feishu]`，但对应的功能未编译进去，`synapseclaw channel list`、`synapseclaw channel doctor` 和 `synapseclaw channel start` 会报告该渠道在此构建中被故意跳过。
...
access_token = \"syt_...\"
user_id = \"@synapseclaw:matrix.example.com\"   # 可选，推荐用于 E2EE
...
- `SYNAPSECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` 会覆盖配置中的密钥。
...
- `SYNAPSECLAW_LINQ_SIGNING_SECRET` 会覆盖配置中的密钥。
