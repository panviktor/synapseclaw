...
1. `SYNAPSECLAW_WORKSPACE` 覆盖（如果设置）
2. 持久化的 `~/.synapseclaw/active_workspace.toml` 标记（如果存在）
3. 默认 `~/.synapseclaw/config.toml`

SynapseClaw 在启动时以 `INFO` 级别记录解析后的配置：

- `Config loaded` 包含字段：`path`、`workspace`、`source`、`initialized`

模式导出命令：

- `synapseclaw config schema`（将 JSON Schema 草案 2020-12 打印到 stdout）
...
| `otel_service_name` | `synapseclaw` | 发送到 OTLP 收集器的服务名称 |
...
- 查询运行时跟踪：
  - `synapseclaw doctor traces --limit 20`
  - `synapseclaw doctor traces --event tool_call_result --contains \"error\"`
  - `synapseclaw doctor traces --id <trace-id>`
...
1. `SYNAPSECLAW_PROVIDER`（显式覆盖，非空时始终优先）
...
- 当你有意让运行时环境覆盖非默认配置的提供商时，请使用 `SYNAPSECLAW_PROVIDER`。
...
- 当 `enabled = true` 且不存在 OTP 密钥时，SynapseClaw 会生成一个并打印一次注册 URI。
...
| `state_file` | `~/.synapseclaw/estop-state.json` | 持久化 estop 状态路径 |
...
- 使用 CLI 命令 `synapseclaw estop` 启动，`synapseclaw estop resume` 清除级别。
...
- 安全优先默认：除非 `open_skills_enabled = true`，否则 SynapseClaw **不会**克隆或同步 `open-skills`。
- 环境覆盖：
  - `SYNAPSECLAW_OPEN_SKILLS_ENABLED` 接受 `1/0`、`true/false`、`yes/no`、`on/off`。
  - `SYNAPSECLAW_OPEN_SKILLS_DIR` 非空时覆盖仓库路径。
  - `SYNAPSECLAW_SKILLS_PROMPT_MODE` 接受 `full` 或 `compact`。
- 启用标志的优先级：`SYNAPSECLAW_OPEN_SKILLS_ENABLED` → `config.toml` 中的 `skills.open_skills_enabled` → 默认 `false`。
- 建议在低上下文本地模型上使用 `prompt_injection_mode = \"compact\"`，以减少启动提示大小，同时按需保留技能文件可用。
- 技能加载和 `synapseclaw skills install` 都会应用静态安全审计。包含符号链接、类脚本文件、高风险 shell  payload 片段或不安全 markdown 链接遍历的技能会被拒绝。
...
- SynapseClaw 请求 Composio v3 工具时使用 `toolkit_versions=latest`，并使用 `version=\"latest\"` 执行工具，以避免过时的默认工具版本。
...
3. 在重启/部署前使用 `synapseclaw doctor` 验证。
...
- 当 `synapseclaw channel start` 运行时，`default_provider`、`default_model`、`default_temperature`、`api_key`、`api_url` 和 `reliability.*` 的更新会在下一条入站消息时从 `config.toml` 热应用。
...
- 设置时 `SYNAPSECLAW_LINQ_SIGNING_SECRET` 覆盖 `signing_secret`。
...
- 设置时 `SYNAPSECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` 覆盖 `webhook_secret`。
