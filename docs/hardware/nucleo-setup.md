# SynapseClaw on Nucleo-F401RE — Step-by-Step Guide

Run SynapseClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

SynapseClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
SynapseClaw includes everything for Nucleo-F401RE:
...
| Flash command | `synapseclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via SynapseClaw

From the synapseclaw repo root:
...
Add to `~/.synapseclaw/config.toml`:
...
| 5 | `synapseclaw daemon` or `synapseclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `synapseclaw peripheral list` to verify.
