# SynapseClaw on Arduino Uno Q — Step-by-Step Guide

Run SynapseClaw on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

SynapseClaw includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `synapseclaw peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
## Phase 2: Install SynapseClaw on Uno Q
...
# Clone synapseclaw (or scp your project)
...
## Phase 4: Run SynapseClaw Daemon
...
**At this point:** Telegram chat works. Send messages to your bot — SynapseClaw responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (SynapseClaw Handles It)

SynapseClaw includes the Bridge app and setup command.

### 5.1 Deploy Bridge App

**From your Mac** (with synapseclaw repo):
```bash
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, SynapseClaw uses `gpio_write` via the Bridge.
...
| 8 | Edit `~/.synapseclaw/config.toml` (add Telegram bot_token) |
| 9 | `synapseclaw daemon --host 127.0.0.1 --port 42617` |
...
- **"command not found: synapseclaw"** — Use full path: `/usr/local/bin/synapseclaw` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`synapseclaw peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. SynapseClaw uses the correct v4 endpoint.
