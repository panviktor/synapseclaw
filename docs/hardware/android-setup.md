...
SynapseClaw provides prebuilt binaries for Android devices.
...
The easiest way to run SynapseClaw on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/panviktor/synapseclaw/releases/latest/download/synapseclaw-aarch64-linux-android.tar.gz
tar xzf synapseclaw-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/panviktor/synapseclaw/releases/latest/download/synapseclaw-armv7-linux-androideabi.tar.gz
tar xzf synapseclaw-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run SynapseClaw outside Termux:

```bash
# From your computer with ADB
adb push synapseclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/synapseclaw
adb shell /data/local/tmp/synapseclaw --version
```
