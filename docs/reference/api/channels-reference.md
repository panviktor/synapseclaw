...
This document is the canonical reference for channel configuration in SynapseClaw.
...
6. **Stale runtime state**: config changed but `synapseclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.synapseclaw/config.toml`.
...
When running `synapseclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `synapseclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

SynapseClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `synapseclaw channel list`, `synapseclaw channel doctor`, and `synapseclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@synapseclaw:matrix.example.com"   # optional, recommended for E2EE
...
SynapseClaw supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.synapseclaw/state/whatsapp-web/session.db"
...
port = 6697
nickname = "synapseclaw-bot"
username = "synapseclaw"              # optional
channels = ["#synapseclaw"]
allowed_users = ["*"]
...
- `SYNAPSECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `SYNAPSECLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `synapseclaw daemon` after config changes.
...
```bash
RUST_LOG=info synapseclaw daemon 2>&1 | tee /tmp/synapseclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/synapseclaw.log
```
