# SynapseClaw Providers Reference
...
- After multimodal normalization, SynapseClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, SynapseClaw returns a structured capability error instead of silently ignoring images.
...
- SynapseClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `synapseclaw models refresh --provider llamacpp`
...
- Model discovery: `synapseclaw models refresh --provider sglang`
...
- Model discovery: `synapseclaw models refresh --provider vllm`
...
- Model discovery: `synapseclaw models refresh --provider osaurus`
...
- Model discovery: `synapseclaw models refresh --provider nvidia`
