...
This guide covers native Nextcloud Talk integration for SynapseClaw.
...
Add this section in `~/.synapseclaw/config.toml`:
...
synapseclaw daemon
# or
synapseclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, SynapseClaw verifies:
...
- SynapseClaw ignores bot-originated webhook events (`actorType = bots`).
- SynapseClaw ignores non-message/system events.
...
3. Confirm SynapseClaw receives and replies in the same room.
