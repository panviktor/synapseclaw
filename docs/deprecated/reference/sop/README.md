...
- CLI `synapseclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.synapseclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.synapseclaw/workspace/sops/deploy-prod/SOP.md
   ```

3. Validate and inspect definitions:

   ```bash
   synapseclaw sop list
   synapseclaw sop validate
   synapseclaw sop show deploy-prod
   ```
