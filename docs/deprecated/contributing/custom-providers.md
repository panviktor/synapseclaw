...
SynapseClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export SYNAPSECLAW_API_KEY="your-api-key"
synapseclaw agent
```

## llama.cpp Server (Recommended Local Setup)

SynapseClaw includes a first-class local provider for `llama-server`:
...
```bash
synapseclaw models refresh --provider llamacpp
synapseclaw agent -m "hello"
```

You do not need to export `SYNAPSECLAW_API_KEY=dummy` for this flow.

## SGLang Server

SynapseClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
synapseclaw models refresh --provider sglang
synapseclaw agent -m "hello"
```
...
SynapseClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
synapseclaw models refresh --provider vllm
synapseclaw agent -m "hello"
```
...
# Single message test
synapseclaw agent -m "test message"
```
