...
- `cargo check` / `cargo build` appears stuck at `Checking synapseclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in SynapseClaw:
...
### `synapseclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `synapseclaw`
...
Verify `~/.synapseclaw/config.toml`:
...
- stop extra `synapseclaw daemon` / `synapseclaw channel start` processes
