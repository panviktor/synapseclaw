...
- `cargo check` / `cargo build` dừng lâu ở `Checking synapseclaw`
...
### Không tìm thấy lệnh `synapseclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `synapseclaw`
...
Xác minh `~/.synapseclaw/config.toml`:
...
- Dừng các tiến trình `synapseclaw daemon` / `synapseclaw channel start` thừa
