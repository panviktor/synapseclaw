# Triển khai mạng — SynapseClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai SynapseClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | SynapseClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | SynapseClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — SynapseClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. SynapseClaw trên Raspberry Pi
...
Chỉnh sửa `~/.synapseclaw/config.toml`:
...
- Channel Telegram hoạt động: SynapseClaw poll Telegram API (outbound)
...
```bash
synapseclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `synapseclaw tunnel` (xem tài liệu tunnel).

3. SynapseClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- SynapseClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `synapseclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `synapseclaw daemon`).
...
- [ ] Chạy `synapseclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
