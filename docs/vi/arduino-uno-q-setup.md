# SynapseClaw trên Arduino Uno Q — Hướng dẫn từng bước

Chạy SynapseClaw trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

SynapseClaw bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `synapseclaw peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt SynapseClaw trên Uno Q
...
# Clone synapseclaw (hoặc scp project của bạn)
...
# Cài đặt
sudo cp target/release/synapseclaw /usr/local/bin/
```
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/synapseclaw arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/synapseclaw /usr/local/bin/"
```
...
# Cấu hình nhanh
synapseclaw onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Hoặc tạo config thủ công
mkdir -p ~/.synapseclaw/workspace
nano ~/.synapseclaw/config.toml
```
...
## Phase 4: Chạy SynapseClaw Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Chạy daemon (Telegram polling hoạt động qua WiFi)
synapseclaw daemon --host 127.0.0.1 --port 3000
```

**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — SynapseClaw phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (SynapseClaw xử lý tự động)

SynapseClaw bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo synapseclaw):
```bash
synapseclaw peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
synapseclaw peripheral setup-uno-q
```
...
### 5.3 Chạy SynapseClaw

```bash
synapseclaw daemon --host 127.0.0.1 --port 3000
```

Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, SynapseClaw dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/panviktor/synapseclaw.git && cd synapseclaw` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `synapseclaw onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.synapseclaw/config.toml` (thêm Telegram bot_token) |
| 9 | `synapseclaw daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: synapseclaw"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/synapseclaw` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`synapseclaw peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. SynapseClaw dùng endpoint v4 chính xác.
