# Thêm Board và Tool — Hướng dẫn phần cứng SynapseClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào SynapseClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.synapseclaw/config.toml)
synapseclaw peripheral add nucleo-f401re /dev/ttyACM0
synapseclaw peripheral add arduino-uno /dev/cu.usbmodem12345
synapseclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, SynapseClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `synapseclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `synapseclaw peripheral list` | Liệt kê các board đã cấu hình |
| `synapseclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `synapseclaw peripheral flash` | Nạp firmware Arduino |
| `synapseclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `synapseclaw hardware discover` | Liệt kê thiết bị USB |
| `synapseclaw hardware info` | Thông tin chip qua probe-rs |
