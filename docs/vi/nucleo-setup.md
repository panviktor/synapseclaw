# SynapseClaw trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy SynapseClaw trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

SynapseClaw có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
SynapseClaw bao gồm mọi thứ cần thiết cho Nucleo-F401RE:
...
| Flash command | `synapseclaw peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
### 1.2 Nạp qua SynapseClaw

Từ thư mục gốc của repo synapseclaw:
...
Thêm vào `~/.synapseclaw/config.toml`:
...
```bash
synapseclaw agent --message "Turn on the LED on pin 13"
```
...
| 3 | `synapseclaw peripheral flash-nucleo` |
| 4 | Thêm Nucleo vào config.toml (path = serial port của bạn) |
| 5 | `synapseclaw daemon` hoặc `synapseclaw agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `synapseclaw peripheral list` để xác nhận.
