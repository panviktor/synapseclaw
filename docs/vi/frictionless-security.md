...
[INFO] ✓ Audit logging enabled (~/.config/synapseclaw/audit.log)
...
```toml
# ~/.config/synapseclaw/config.toml
...
$ synapseclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/synapseclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ synapseclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ synapseclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ synapseclaw config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `synapseclaw security --status`
