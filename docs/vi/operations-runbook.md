...
|---|---|---|
| Foreground runtime | `synapseclaw daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `synapseclaw gateway` | kiểm thử webhook endpoint |
| User service | `synapseclaw service install && synapseclaw service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `synapseclaw doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `synapseclaw channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `synapseclaw status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.synapseclaw/daemon_state.json` | file được cập nhật định kỳ |
...
4. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.synapseclaw/config.toml`.
...
1. sao lưu `~/.synapseclaw/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `synapseclaw doctor`
