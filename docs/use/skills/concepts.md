...
| Type | Who creates it | Default status | Stored where | Can be edited | Can be auto-promoted |
| --- | --- | --- | --- | --- | --- |
| User-authored | Human | Active manual path | Memory | Yes | No |
| Generated new | System | Candidate | Memory | Yes, after review | Limited by policy |
| Generated patch | System | Candidate | Candidate queue and memory records | Applied to target skill | Policy-gated |
| Imported package | File or open-skills source | Governed/indexed | File-backed with compact memory card | Package flow | No |
| Ported legacy | Old workspace `SKILL.*` package | Memory-backed after port | Memory plus `skills/ported/` archive | Yes | No |
